<template>
    <v-container fluid class="review-card-manage pa-4">
        ...
        <div class="d-flex flex-wrap align-center mb-2" style="gap: 4px;">
            ...
            <review-card-leech-mutation-surface
                ref="leechMutationSurface"
                :run-lifecycle-action="runLeechLifecycleAction"
                :run-bulk-lifecycle="runLeechBulkLifecycle"
                @state-change="onLeechStateChange"
                @refresh="loadData"
        ...
        <review-card-table-surface
            ...
            :bulk-rewrite-loading="leechSurfaceState.bulkRewriteLoading"
            :bulk-leech-suspend-loading="leechSurfaceState.bulkSuspendLoading"
            ...
            @marker-change="setMarker"
            @bulk-marker="setBulkMarker"
            @notify="showSnackbar"
        />
        ...
        <review-card-marker-mutation-surface
            ref="markerMutationSurface"
            @card-updated="onMarkerCardUpdated"
            @bulk-updated="onMarkerBulkUpdated"
            @error="onMarkerError"
        />
...
import ReviewCardLeechMutationSurface from './ReviewCardLeechMutationSurface.vue';
import ReviewCardMarkerMutationSurface from './ReviewCardMarkerMutationSurface.vue';
...
export default {
    components: {
        ...
        ReviewCardLeechMutationSurface,
        ReviewCardMarkerMutationSurface,
    },
    props: {
        language: {
            type: String,
    ...
    methods: {
        ...
        onLeechStateChange(state) {
            this.leechSurfaceState = { ...state };
        },
        runLeechLifecycleAction(payload) {
            const surface = this.$refs.lifecycleMutationSurface;
            return surface && typeof surface.runLifecycleAction === 'function'
                ? surface.runLifecycleAction(payload)
                : Promise.reject(new Error('lifecycle_surface_unavailable'));
        },
        runLeechBulkLifecycle(payload) {
            const surface = this.$refs.lifecycleMutationSurface;
            return surface && typeof surface.runBulkLifecycle === 'function'
                ? surface.runBulkLifecycle(payload)
                : Promise.reject(new Error('lifecycle_surface_unavailable'));
        },
        openRewritePackageDialog(item) {
            this.$refs.leechMutationSurface?.openRewritePackageDialog(item);
        },
        suspendLeechCard(item) {
            this.$refs.leechMutationSurface?.suspendCard(item);
        },
        openBulkRewritePackages(selection) {
            this.$refs.leechMutationSurface?.openBulkRewritePackages(selection);
        },
        confirmBulkLeechSuspend(selection) {
            this.$refs.leechMutationSurface?.confirmBulkSuspend(selection);
        },
        setMarker({ item, marker }) {
            this.$refs.markerMutationSurface?.setMarker(item, marker);
        },
        setBulkMarker(selection) {
            this.$refs.markerMutationSurface?.setBulkMarker(selection, selection.marker);
        },
        onMarkerCardUpdated(payload) {
            const item = this.items.find(card => card.review_card_id === payload.review_card_id);
            if (item) this.$set(item, 'marker', payload.marker);
            this.showSnackbar('标记已更新。', 'success');
        },
        onMarkerBulkUpdated(payload) {
            const applied = new Set(payload.applied_ids || []);
            this.items.forEach((item) => {
                if (applied.has(item.review_card_id)) this.$set(item, 'marker', payload.marker);
            });
            this.clearTableSelection();
            this.showSnackbar(`已更新 ${applied.size} 张卡的标记。`, 'success');
        },
        onMarkerError(message) {
            this.error = message;
        },
