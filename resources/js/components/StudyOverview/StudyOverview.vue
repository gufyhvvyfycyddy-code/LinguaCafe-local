<template>
    <v-container id="study-overview" fluid class="overview-shell py-6">
        ...
        <v-card outlined class="rounded-lg pa-4 mb-5"><v-row align="center">
            <v-col cols="12" md="10"><v-btn-toggle v-model="period" mandatory color="primary" @change="load"><v-btn v-for="value in [30,90,365]" :key="value" :value="value">{{ value }} 天</v-btn></v-btn-toggle></v-col>
...
export default{components:{SectionTitle,MetricCard,MetricLine,DistributionCard},data:()=>({loading:false,error:'',data:null,period:30}),computed:{
metaText(){return this.data?`${this.data.meta.language} · 全部已确认词义卡 · ${this.data.meta.period} 天 · ${this.data.meta.scope_card_count} 张卡`:'当前语言全部已确认词义卡的只读学习统计';},
...
mounted(){this.load();},methods:{load(){this.loading=true;this.error='';axios.get('/study-overview/data',{params:{period:this.period}}).then(({data})=>{this.data=data;}).catch(e=>{this.error=e.response?.data?.message||'学习总览加载失败。';}).finally(()=>{this.loading=false;});},delta(key){return this.data.today.override?.[key]||0;},barWidth(value,max){return`${Math.max(value?3:0,value*100/max)}%`;},num(value){return value===null?'Unavailable':Number(value).toFixed(2);},percent(value){return value===null?'Unavailable':`${(Number(value)*100).toFixed(1)}%`;},formatDuration(value){if(value===null)return'Unavailable';const seconds=Math.round(Number(value)/1000);return seconds<60?`${seconds} 秒`:`${Math.floor(seconds/60)}分 ${seconds%60}秒`;},retentionText(value,count){return value===null?`Unavailable (${count})`:`${Number(value).toFixed(1)}% (${count})`;}}};
</script>
<style scoped>
