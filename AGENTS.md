...
所有高风险任务都必须先完成架构审查。普通任务在用户明确确认后实施；目标模式可使用“目标模式阶段预授权”：当用户明确创建或恢复一个指向权威 roadmap 或有序里程碑的持续目标时，目标授权分阶段工作流。只有架构审查在目标点名范围内冻结当前切片的具体范围，并仅用接受的计划、ADR、契约和测试作为约束与证据，且不存在未解决的强制停止项时，实施授权才生效；完成相称验证和逐项完成审计后，方可进入下一切片。

目标模式阶段预授权不得覆盖新发现的强制停止项：破坏性或生产数据操作；migration 执行、新表或数据回填；未冻结的数据模型或公开 API/payload 语义；FSRS、正式评分、ReviewLog、WordSense 绑定、ReviewCard 身份或 lifecycle 语义；真实 AI provider、密钥、外发、付费、模型或成本上限；多个未审查 seam；未批准的新 ADR；未决产品选择、权威冲突或目标外扩张。只有用户明确批准的具体决定并入当前目标；批准 migration 计划不等于批准 migration 执行，也不自动授权相邻决定。权威、范围、风险、数据处理、外发、成本假设或实现发生实质变化时必须重新确认。依据见 ADR-0028。

以下情况若为新发现、未冻结或尚未作为具体决定纳入当前目标授权，必须立即停止并请求新的确认：
