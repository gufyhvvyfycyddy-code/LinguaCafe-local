...
class ReadingUnfamiliarTargetService
{
    ...
    public function syncClientSnapshot(
        int $userId,
        string $language,
        int $chapterId,
        array $targets,
        ...
        return DB::transaction(function () use ($userId, $language, $chapterId, $targets, $expectedSnapshotVersion) {
            ...
            $currentVersion = $this->snapshotVersion($currentTargets, $sourceRevision);
            if (!$expectedSnapshotVersion || !hash_equals($currentVersion, $expectedSnapshotVersion)) {
                throw ValidationException::withMessages([
                    'marked_targets_snapshot_version' => ['READING_MARKED_TARGET_SNAPSHOT_STALE'],
                ]);
            }
            ...
            $savedTargets = $this->currentModels($userId, $language, $chapterId, $sourceRevision);

            return [
                'source_revision' => $sourceRevision,
                'snapshot_version' => $this->snapshotVersion($savedTargets, $sourceRevision),
                'targets' => array_values(array_map(fn (array $canonical) => [
                    'occurrence_id' => $canonical['occurrence_id'],
                    'kind' => $canonical['kind'],
                    'start_word_index' => $canonical['start_word_index'],
                    'end_word_index' => $canonical['end_word_index'],
