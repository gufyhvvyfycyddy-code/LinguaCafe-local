...
/**
 * SenseReviewThirtyDayCalendarService
 *
 * SenseReview-ThirtyDayCalendar-1000-1
 *
 * Read-only "近 30 天复习日历" service. Fixed rolling 30-day window:
 * today + previous 29 natural days (NOT a natural month). Source of truth:
 ...
 *    30-day window — constant query budget regardless of sense count.
 ...
 *  - Today's row matches DailyReport.
 *  - Last 7 days match SevenDayTrend.
 *
 * Layering:
 *   Period Layer   → rollingDays(30, tz) gives start/end/day_keys
 ...
 */
class SenseReviewThirtyDayCalendarService
{
    ...
    /**
     * Build the read-only 30-day calendar payload.
     *
     * @param  int     $userId
    ...
    public function build(int $userId, string $language): array
    {
        ...
        // Period Layer: compute the fixed 30-day window.
        $period = $this->periodService->rollingDays(30, $timezone);

        // SINGLE query for the whole 30-day window via the centralized Query Layer.
        $logs = $this->analytics->reviewsForPeriod($userId, $language, $period['start'], $period['end']);

        // Series Layer: zero-fill to exactly 30 ascending day entries.
    ...
    /**
     * Build the summary block for the whole 30-day window.
     *
     * average_per_active_day = total_reviews / active_days, null when
     * active_days = 0 (not a fake 0).
     *
     * @param  Collection  $logs  All logs in the 30-day window.
     * @param  array       $days  The 30 zero-filled day entries.
     * @return array{
     *   total_reviews: int,
     *   active_days: int,
     *   distinct_senses: int,
