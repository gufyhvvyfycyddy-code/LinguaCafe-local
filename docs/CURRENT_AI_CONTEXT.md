...
>
> **2026-08-29 current overlay**：当前工作不再由旧 recovery `CURRENT_MILESTONE.json` 或旧 Anki-aligned roadmap 决定。当前产品权威仍是 `docs/product/LinguaCafe_Product_Rebaseline_English_Reading_First_2026-08-18.md`。Goal plan 已完成 Phase G/G-GATE 与 `H-00`–`H-09`，并已完成当前主机可执行的 **H-11 final Web + Android regression**。H-11 product/runtime repair commit=`eaab88a7a7d96d2c1078b4b5243210430a305970`，报告见 `docs/testing/h11-final-web-android-regression-acceptance-2026-08-29.md`；其证据包括 PHP 8.4 Unit+Feature 4034/19397、JS 477/477、mobile 42/42 + build、生产 Docker native FSRS 真 build/load/schedule、真实 Web Reader→manual Sense→Finish→Review/WordSense、430px 四主入口，以及 Android 16 login→offline/reconnect→device revoke。H-10 iOS capability cluster 仍保持 **DEFERRED / Not Complete**：当前主机为 Windows，无 Xcode/xcrun/codesign/Apple signing，无在线 macOS peer；Windows-static 42 mobile tests、当前 iOS generated-Web integrity 与修复后的 M9 source guard 均通过，但这些证据不替代真实 Xcode build/device/Keychain/TestFlight。H-10 guard repair commit=`5f60bc9557dcdbbc79fe79928581a19af51052c5`，报告见 `docs/testing/h10-ios-capability-probe-2026-08-29.md`。当前 forward milestone 为 **H-GATE final Goal completion audit**；H-GATE 必须保留 deferred iOS capability，不能因 Web/Android 已通过而声明完整跨平台 Goal 完成。用户已明确启用单窗口直接执行，因此当前窗口可连续推进已命名 milestone；fixed DIRECT/四窗口流程仅在用户重新启用并行模式时恢复。Reader 的机会式提前 Good、跨文章/跨 session 的完整 24h 正向最小间隔、同 session/card 单次计分、existing-Sense“不认识”→Again 继续以 ADR-0061 与 ADR-0063 为准；AI matched-existing source binding/full-pool rotation 继续以 ADR-0062 与 ADR-0064 为准。
