...
1. 网页端 GPT 给 OpenCode / CodeBuddy / WorkBuddy 的提示词，必须显式写出"本轮使用 skill"。
2. 不能只写岗位角色。
3. 至少写一个主 skill；复杂任务可以写辅助 skill。
4. skill 必须写在提示词正文里，让智能体执行时能看到。
5. CodeBuddy 仍必须从现有 skills 列表中选择（详见 14.5）。
6. OpenCode 也必须根据任务选择适配 skill，例如 documentation-and-adrs、context-engineering、doubt-driven-development、improve-codebase-architecture、api-and-interface-design、code-review-and-quality。
7. WorkBuddy 如果没有代码类 skill，也必须写明体验 / QA 使用的工作方法或适配 skill；不能空缺。
8. 如果某个任务确实不适合某个 skill，必须在提示词外说明原因。
9. 最终报告必须说明本轮是否按指定 skill 执行。
10. 网页端 GPT 如果漏写 skill，视为提示词不完整，应在下一轮补规则或重发提示词。
