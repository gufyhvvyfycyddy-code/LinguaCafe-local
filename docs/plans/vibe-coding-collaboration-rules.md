...
| 17-21 | notification script、~~WorkBuddy~~（已停用）、复杂度、oh-my-opencode-slim、网页端自动推进 |
...
## 4.x ~~CodeBuddy 风险角色与总设计师反驳核验规则~~（已停用 2026-07-13）

> **⚠️ 已停用：** 本节描述的 CodeBuddy 风险角色已随 CodeBuddy / WorkBuddy 工作流取消而停用。当前工作流见 §1.5 GLM 单 Agent 闭环规则。本节保留为历史参考，不再适用于当前任务。不再生成 CodeBuddy 提示词，不再安排 CodeBuddy 后置复核。

### 4.x.1 ~~CodeBuddy 风险角色定位~~（历史）
...
## 14. ~~当前三员工工作流（2026-06-30）~~（已停用 2026-07-13）

> **⚠️ 已停用：** 本节描述的三员工工作流（CodeBuddy / WorkBuddy / OpenCode 接力棒）已正式取消。当前工作流见 §1.5 GLM 单 Agent 闭环规则。本节保留为历史参考，不再适用于当前任务。不再生成 CodeBuddy / WorkBuddy 提示词，不再要求三员工接力棒，不再要求多文本框分员工。
...
### 14.16 主线关键路径与小步骤双轨并行规则

> **问题来源**：Queue Order 复合主线任务发现，现有规则没有明确区分"主线关键路径"和"支撑小步骤"的并行关系，容易导致两种偏差：(1) 把所有支撑小步骤当成主线前置条件，主线被文档润色阻塞；(2) 主线完成后才集中做支撑，导致 SUPPORT 数量膨胀冒充主线完成。

#### 14.16.1 核心定义

1. **主线关键路径（ARCH-/DEV-）**：任务规格中明确要求的架构优化、后端开发、前端开发、测试先行等关键路径。主线一旦具备开始条件，必须立即推进。
2. **支撑小步骤（SUPPORT-）**：文档修正、规则同步、轻量测试、guard、注释整理、静态核验、ADR 修正、handoff 更新等低风险工作。支撑小步骤的目的是提高主线质量，而不是制造工作量。
3. **验证与验收（VERIFY-）**：自动化测试、Node guard、npm build、db:doctor、MCP Chrome 真实验收、git commit/push、最终报告。

#### 14.16.2 双轨并行规则

1. 复杂主线可以由大量低风险、可独立核验的小步骤组成。
2. 单个步骤简单，不代表整个任务应拆成简单微任务。
3. 文档修正、规则同步、轻量测试、guard、注释和静态核验优先搭载在相关主线任务中完成。
4. 主线架构或开发一旦具备开始条件，必须立即推进。
5. 支撑小步骤不得成为主线的虚假前置条件。
6. 只要主线仍有可执行工作，就不得为了完成文档润色、格式整理或广泛扫描而暂停主线。
7. 独立、无共享写入冲突的读取、测试、文档核对可以并行。
8. 同一文件、同一数据库状态或同一关键契约上的修改必须串行。
9. 禁止并行修改同一文件。
10. 禁止并行运行会争用同一测试数据库的数据写入测试。

#### 14.16.3 小步骤失败处理

1. 小步骤失败时：
   - 若不影响安全和核心契约，记录 Incomplete 并继续主线；
   - 若证明主线架构错误或有数据风险，暂停相关主线部分。
2. 支撑小步骤失败但不影响安全时，记录并继续主线，不得因为 SUPPORT 阻塞 ARCH/DEV。
3. 例外：SUPPORT 发现以下问题时必须暂停相关主线部分：
   - 安全边界错误；
   - 数据写入风险；
   - API 契约冲突；
   - 测试证明架构假设错误。

#### 14.16.4 To-do list 与报告要求
...
1. 禁止为了凑复杂度加入无意义的小步骤。
2. 禁止把多个无关高风险任务强行并行。
3. 小步骤的目的应是提高主线质量，而不是制造工作量。
4. 任务完成后仍然不得自动进入下一主线。
5. 不得用 SUPPORT 数量冒充主线完成。

#### 14.16.6 与现有规则的关系

1. 本规则不替代 §14.12（OpenCode 任务打包规则），而是补充主线与支撑的并行关系。
2. 本规则不替代 §19（复杂度规则），复杂度 100 任务仍需遵守 §19.3 的边界约束。
3. 本规则不替代 §22.5（零进度任务不得单独派发），零进度任务仍不得单独派发。
4. 本规则不覆盖任何安全红线。
...
## 18. ~~WorkBuddy 单专家规则~~（已停用 2026-07-13）

> **⚠️ 已停用：** 本节描述的 WorkBuddy 单专家规则已随 CodeBuddy / WorkBuddy 工作流取消而停用。当前工作流见 §1.5 GLM 单 Agent 闭环规则。本节保留为历史参考，不再适用于当前任务。不再生成 WorkBuddy 提示词，不再要求指定内置专家。
...
- 本条规则**只约束网页端总流程设计师**（网页端 GPT），不授权本地 Agent 自动进入下一任务。
- OpenCode 仍然必须在完成当前任务后停止，并在报告中写"是否进入下一任务：否"。
- CodeBuddy 仍然必须只做事实复核，不进入下一任务。
- WorkBuddy 仍然必须只做当前专家验收，不进入下一任务。
- 网页端总流程设计师可以自动生成下一阶段提示词，但任务是否执行仍由用户复制给本地 Agent。

### 21.7 不覆盖其它硬规则

本条规则不覆盖 LinguaCafe 原有禁止规则：
- OpenCode 后置 CodeBuddy 仍然有效；
- 涉及页面后置 WorkBuddy 仍然有效；
- MCP Chrome 真实验收规则仍然有效；
- DCP 默认禁止；
- notification script 默认禁止；
- `.env` 禁止读取/修改；
...
### 21.8 oh-my-opencode-slim 要求

从本规则写入后，网页端 GPT 每次生成 OpenCode 提示词，都必须包含：
...
每次网页端总设计师给出 OpenCode / Codex / Trae / CodeBuddy / WorkBuddy 提示词之前，必须先给用户一个简短进度说明。这防止总设计师自身丢失上下文，也让用户了解当前阶段。
