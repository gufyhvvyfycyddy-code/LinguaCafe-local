...
2. **WorkBuddy 不使用 CodeBuddy / OpenCode 那套 skills。** WorkBuddy 有自己的内置专家机制，提示词应写明"本轮使用 WorkBuddy 内置专家"，例如：
   - 产品经理
   - 网页端体验官
   - QA
   - 用户流程验收员
3. 至少写一个主 skill（OpenCode / CodeBuddy）或一个内置专家（WorkBuddy）；复杂任务可以写辅助 skill 或多名专家。
...
### 14.10 本地登录与管理员测试账号规则

1. LinguaCafe 是本地项目，账号主要用于本地区分用户和完成本地验收。
2. 如果本地浏览器 smoke / 手动验收因为登录不上而卡住，OpenCode 可以创建用户指定的本地管理员测试账号。
3. 具体账号和密码只允许出现在当前任务提示词中，不写入 GitHub 文档、代码、测试、日志或最终报告。
4. 文档只写"使用当前任务提示词提供的账号和密码"，不要写具体邮箱和密码。
5. 禁止修改 .env。
6. 禁止把密码 commit 到仓库。
7. 创建账号失败时只报告原因，不允许清库。
8. 禁止 migrate:fresh / db:wipe / 清库。
9. 该账号只用于本地验收，不代表线上账号或公网账号。
