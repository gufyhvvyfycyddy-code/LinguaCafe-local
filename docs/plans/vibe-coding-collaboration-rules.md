...
> **问题来源**：Task 2000-16。历史任务复杂度出现过 20、35、40、60 等低于 100 的值，导致 GLM 在充分核查、测试和自审上的预算被压缩，影响交付质量。本规则把 GLM 执行任务的复杂度下限固定为 100，确保每个任务都有足够预算做完整事实核查、red→green TDD、MCP Chrome 验收（如适用）、文档一致性搜索、FACT 自审和最终报告。

1. **所有 GLM 执行任务的复杂度最低为 100**。无论是代码任务、文档任务、架构契约任务还是验收任务，复杂度都不允许低于 100。
2. **后续提示词中的复杂度不得写成 20、35、40、60 等低于 100 的值**。出现低于 100 的复杂度声明时，GLM 仍按 100 最低预算执行，并在报告开头标记"提示词复杂度低于 100，已按最低 100 执行"。
3. **简单文档任务同样写复杂度 100**。文档与架构契约任务虽然不写代码，但仍需要：基线核查、文档一致性搜索、GlmSingleAgentWorkflowDocsGuard、git diff --check、FACT 自审，预算不能低于 100。
4. **复杂度 100 表示允许 GLM 进行充分核查、测试和自审**，包括但不限于：
...
6. **如果以后需要更复杂，可以使用 120、150 或更高**，但不得低于 100。复杂度提升到 120 / 150 时，应明确说明额外预算用于哪些环节（例如：更深的架构侦察、更广的回归测试、跨阶段一致性核查）。
7. **模型和档位推荐继续写在 GLM 提示词正文之外**，不混入复杂度数值。复杂度只表达任务预算下限，不指定模型档位。
...
- 架构重构 + 契约测试 + GLM 内部 FACT/UX 轨道复核的全链路重构。
- 多阶段但边界清楚的功能收口任务。
