...
> **日期**：2026-07-16
> **基线 commit**：`4968d00830ee928f2495dd4046d01c2a5337ffa3`
...
Settings 架构收敛和 Preset V1A–V1D 已生产关闭。V1D 已通过纯状态模块、分组回归、双用户、English/French、CRUD、共享修改、刷新持久化、删除重绑定、数据库 delta 和 Chrome 双 viewport 验收。下一授权阶段是 Browser / ReviewCardManage 架构收敛；其 Phase 3A — Card Info Drawer Extraction 已完成目标冻结和 Codex 提示词准备，但代码尚未开始。详细边界见 `docs/plans/review-card-manage-architecture-convergence-plan.md`。Card Marker / Custom Study 1B、Reviewer、Reader 与真实 AI provider 继续按顺序延后。
...
| 文件 | 行数 | 结构信号 | 风险 |
|---|---:|---|---|
| `resources/js/components/ReviewCards/ReviewCardManage.vue` | 3,411 | 24 个直接 `axios.` 引用、12 个 `v-dialog`、约 115 个 method/computed 项 | 高 |
...
状态：Authorized Next / Target Prepared / Not Started。第一安全切片为 **Phase 3A — Card Info Drawer Extraction**；代码尚未开始。详细分期、允许文件、禁止范围和验收合同见 `docs/plans/review-card-manage-architecture-convergence-plan.md`。
...
| 2 | Browser / ReviewCardManage 架构收敛 | Authorized Next / Target Prepared / Not Started；Phase 3A Card Info Drawer Extraction 已冻结，当前最大前端热点，也是后续 Marker 和治理入口；见 `review-card-manage-architecture-convergence-plan.md` |
