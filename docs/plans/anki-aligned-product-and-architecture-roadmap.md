...
Settings 架构收敛和 Preset V1A–V1D 已生产关闭。V1D 已通过纯状态模块、分组回归、双用户、English/French、CRUD、共享修改、刷新持久化、删除重绑定、数据库 delta 和 Chrome 双 viewport 验收。Browser / ReviewCardManage Phase 3A — Card Info Drawer Extraction 已实现并通过自动化与 MCP Chrome 功能检查；因当前浏览器接口无法导出 XHR/Network 日志，仍等待网页端最终验收，不进入 Phase 3B。详细边界见 `docs/plans/review-card-manage-architecture-convergence-plan.md`。Card Marker / Custom Study 1B、Reviewer、Reader 与真实 AI provider 继续按顺序延后。
...
| 文件 | 行数 | 结构信号 | 风险 |
|---|---:|---|---|
| `resources/js/components/ReviewCards/ReviewCardManage.vue` | 2,773 | 22 个直接 `axios.` 引用、12 个 `v-dialog`；Card Info 详情请求已由唯一子组件负责 | 高 |
...
状态：**Phase 3A Implemented / Automated Verified / Browser Functional Checks Complete / Network Trace Pending / Awaiting web-side final acceptance**。Card Info 抽屉已完成单一子组件职责抽取；Phase 3B 未授权。详细分期、允许文件、禁止范围和验收合同见 `docs/plans/review-card-manage-architecture-convergence-plan.md`。
...
| 2 | Browser / ReviewCardManage 架构收敛 | Phase 3A 已实现并通过自动化与 MCP Chrome 功能检查；Network trace 与网页端最终验收待完成，Phase 3B 未授权；见 `review-card-manage-architecture-convergence-plan.md` |
