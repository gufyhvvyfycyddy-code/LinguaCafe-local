...
Settings 架构收敛、Preset V1A–V1D、Browser 授权范围、Card Marker / Custom Study 1B、Reviewer convergence、Reader UI/架构治理和 AI Study Card service convergence 已生产关闭；Phase 3C-3 Delete Mutation Family 仍为 Planned / Not Authorized。Reader 只在实际选词时显示并预留查词侧栏，`TextBlockService` 的不可达 Reader 投影副本已删除。真实 provider 按用户决定保持禁用，不构成当前验收阻塞。
...
| `resources/js/components/ReviewCards/ReviewCardManage.vue` | 872 | 9 个直接 `axios.` 引用、4 个 `v-dialog`；Card Info/Search/Table/Scheduling/Lifecycle/Leech 已有独立所有者 | 中，已成为协调容器 |
...
| `app/Services/AiStudyCardPendingItemService.php` | 195 | V1–V5 compatibility facade; lifecycle/package/generation owners extracted | 低 |
...
- ReviewCardManage 已从 3,411 行降至 905 行，并形成 Card Info、Search、Table、Scheduling、Lifecycle、Leech、Marker 七个职责完整的所有者。
- MCP Chrome 验收流程已建立。

增加维护成本的因素：

- Reader 与 Reviewer 的超大 Vue 页面仍同时承担请求、状态、展示、弹窗和业务编排。
- `TextBlockService` 等热点仍聚集多类职责；`AiStudyCardPendingItemService` 已收敛为兼容 facade，不能重新吸收 lifecycle/package/generation 规则。
- ReviewCardManage 父组件仍有 9 个直接请求和 4 个对话框，并保留无可达表格入口的 legacy `/enabled` 兼容代码；这些属于后续兼容清理债务，不应在当前验收中顺手删除。
...
状态：**Phase 3A、Phase 3B-1、Phase 3B-2、Phase 3C-1、Phase 3C-2、Phase 3C-4 与 Phase 3D Accepted / Production Closed**。Card Info、Search、Table、Scheduling、Lifecycle、Leech 与 Marker 已分别形成单一职责所有者。`ReviewCardManage.vue` 当前为 905 行、9 个 direct `axios.` references、4 个 `v-dialog`；`ReviewCardLeechMutationSurface.vue` 为 312 行，拥有 summary GET、bulk rewrite POST 与两个对话框，生命周期写入仍委托 `ReviewCardLifecycleMutationSurface.vue`。**Phase 3C-3 — Delete Mutation Family：Planned / Not Authorized**，未执行。详细分期和验收合同见 `docs/plans/review-card-manage-architecture-convergence-plan.md`。
...
- **Phase 3C-2 — Lifecycle Mutation Family**：Accepted / Production Closed。`ReviewCardLifecycleMutationSurface.vue` 负责 descriptor、单卡/批量生命周期请求、确认框、冲突处理、状态说明和请求锁；父页面只传递意图与只读快照。Authenticated browser write acceptance 已完成。
- **Phase 3C-3 — Delete Mutation Family**：Planned / Not Authorized。
- **Phase 3C-4 — Leech Governance Mutation Family**：Accepted / Production Closed。Authenticated summary、rewrite、single/bulk suspend、restore 与窄屏验收已完成。
...
状态：**Accepted / Production Closed（2026-07-17）**。优先级：P2。
...
验收结果：共享最小 request coordinator，不合并两个页面；Sense Review 的 `E` / `I` / `-` / `@` / `Ctrl+1..7` 复用现有能力；真实评分后撤销恢复 FSRS 基线，证据见 `docs/testing/reviewer-convergence-browser-acceptance-2026-07-17.md`。

### Phase 6：Reader UI 与阅读架构治理

状态：**Accepted / Production Closed（2026-07-17）**。证据：ADR-0031 与 `reader-attention-and-facade-browser-acceptance-2026-07-17.md`。
...
状态：**Accepted / Real provider deliberately disabled**（ADR-0032）。

优先级：已关闭授权范围；真实 provider 延后到未来单独授权任务。

`AiStudyCardPendingItemService.php` 的高收益职责切片已完成：Pending lifecycle 由 `AiStudyCardPendingLifecycleService` 单独拥有；preview/final package 与 candidate validation/deduplication 由 `AiStudyCardPendingPackageService` 拥有；confirmed card generation 与 source binding 由 `AiStudyCardGenerationService` 拥有。原 Service 仅保留 195 行兼容 facade 委托，未改路由、payload 或 schema，也未做整类重写。

浏览器已证明默认关闭、显式本地路由、失败闭合、无外部浏览器请求和零学习数据写入。请求包已显示 provider/model、item count、外发字段、timeout、成本上限、成本估算可用性、secret 来源类别、失败策略与阻塞原因，且不暴露 secret。用户已决定先跳过真实 provider；只有未来任务重新明确授权时，才需要补齐以下条件：
...
| 2 | Browser / ReviewCardManage 架构收敛 | Phase 3A、3B-1、3B-2、3C-1、3C-2、3C-4 与 3D Accepted / Production Closed；父组件 905 行、9 个请求、4 个弹窗；Phase 3C-3 Planned / Not Authorized；见 `review-card-manage-architecture-convergence-plan.md` |
...
| 7 | AI Study Card service / disabled-provider preflight | 授权范围已关闭；真实 provider 由用户决定保持禁用 |
