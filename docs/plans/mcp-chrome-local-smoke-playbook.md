...
* 当前 fixed DIRECT 明确提供的固定本地测试身份，在 server-bound sentinel 已证明实际验收服务器连接专用 testing 数据库后优先复用；本文档不记录具体凭据。
* 若该固定身份在 testing DB 中不存在，且 DIRECT 明确允许创建同名本地测试身份，则通过应用正常首次设置/注册流程创建**同名** testing 身份。若空库首次设置强制首用户成为管理员，这个 testing-only 首管理员可以用于该 DIRECT 的验收；不得因此执行任务外管理员操作，也不得把它解释为生产权限设计。
* 禁止为了绕过固定身份创建随机替代邮箱。只有当前 DIRECT 没有提供固定身份时，才使用 `codex-acceptance-<unique-marker>@example.test` 创建一个任务专属、最小权限身份；同一批次默认最多一个。
...
- 按 §2 使用当前 DIRECT 的固定 testing 身份；若空库且 DIRECT 允许，则创建同名 testing 身份（包括应用强制的首管理员）；只有 DIRECT 未提供固定身份时才创建最小权限任务身份；
