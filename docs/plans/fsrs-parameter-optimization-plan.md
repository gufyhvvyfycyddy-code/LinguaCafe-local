...
**立即进入 D.3-a：参数优化后端最小实现。**

理由：
1. 所有前置条件满足：fsrs-rs-php 已安装，`compute_parameters()` 可用。
2. ReviewLog 数据完整：rating、reviewed_at、过滤条件均已有。
3. Preflight 已存在：阈值检查逻辑可复用。
4. 无需新增依赖或 migration。
5. D.3-a 只需改 1 个 Service 方法 + 1 个 Controller 方法 + 加测试。

风险低的实现路径：
```
D.3-a（后端优化）→ D.3-c（前端预览）→ D.3-b（确认保存）→ D.3-d（调度集成）
```
