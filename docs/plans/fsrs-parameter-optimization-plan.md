...
> **当前状态**：D.3-a/D.3-c/D.3-b 全部完成，D.2-d（参数来源与版本说明）为下一步
...
**立即进入 D.2-d：参数来源与版本说明。**

D.3-a / D.3-c / D.3-b 全部完成：
...
D.2-d 任务：
1. 替换前端固定文案"fsrs-rs-php 默认参数"为真实来源（'default'/'optimized'/'manual'）。
2. 展示 fsrs_parameters_optimized_at 时间（如"2026-06-25 14:30 优化"）。
3. 纯前端只读，从已有 API 数据中读取。

理由：
1. D.2-d 是 D.3 系列的自然延续：参数已可真实优化和保存。
2. 参数来源字段（fsrs_parameters_source / fsrs_parameters_optimized_at）已存在于 Settings 表。
3. D.2-d 风险低，纯前端展示优化。
