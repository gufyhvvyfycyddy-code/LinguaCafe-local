...
> **当前状态**：D.3-a/D.3-c/D.3-b/D.2-d 全部完成，D.3-d（调度器集成）为下一步
...
2. 如果已保存优化参数 → 传递给 fsrs.rs 构造函数；否则使用 get_default_parameters()。
3. 全面回归测试：ReviewFsrsTest 和 FsrsSchedulingServiceTest 必须继续通过。
