...
> **当前状态**：D.3-c 已完成（前端预览展示，已通过 FSRS-D3-c-recovery 恢复提交），D.3-b（用户确认保存）为下一步
...
**立即进入 D.3-b：用户确认保存优化参数。**

D.3-c 已完成：AdminReviewSettings.vue 前端预览卡片 + 参数对比 + 安全提示。

D.3-b 任务：
1. 后端新增 `confirm: true` 参数，保存 `fsrs_parameters` / `fsrs_parameters_source` / `fsrs_parameters_optimized_at`。
2. 前端增加"确认应用"按钮。
3. 安全校验：确认前再次 verify 参数 count。
4. 测试：保存后 fsrs_parameters !== default parameters。

理由：
1. D.3-c 前端预览已验证 UI 流程完整。
2. API 已返回 optimized_parameters，直接保存即可。
3. D.3-b 是必要的最小保存步骤，风险可控。
