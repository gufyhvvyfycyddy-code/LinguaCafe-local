...
| G-06B | TODO | 阅读机会式提前复习 + 24h 最小正向间隔 + 单次计分：已学 review-state exact Sense 即使 due 尚未来到，在距上一笔有效正式评分满 24 个实际小时后也可因真实阅读回忆提前 Good；不足 24h 无论同篇/跨篇/换 ReadingSession 都只记 exposure；自然识别可 passive Good，主动确认“认识/记得”可 explicit Good；existing Sense 明确“不认识”仍可 exact resolve 后最多一次 Again；同一 ReadingSession/ReviewCard 所有 occurrence 合并为最多一次正式 reading rating | ADR-0061 + existing ReadingSession/settlement + canonical ReviewCard/ReviewLog/FSRS writer | 23h59m positive=0、24h/25h early Good 可成立；external Good→30 秒 Reader Good=0；day-7 / prior day-30 forecast early Good 可成立；同篇同 Sense 10 次最多 1 rating；求助不冒充 positive；unfamiliar→existing 同 session 后续 positive=0；new_sense 不伪造 lapse；无第二 scheduler/reader_due；Web + Mobile offline + concurrency 真验收 |
| G-06C | TODO | AI Reading Assist 语义防重复 + matched-existing 真实来源例句绑定 + saved translation 固定占位：同义/实质近义只能 `matched_existing`；权威确认的真实 Reader occurrence 必须进入该 WordSense 的 `WordSenseOccurrence`/例句轮换池；全文译文/标记词/已学 candidates/paste-back 保持单一 V2 契约；译文显隐不移动英文 | ADR-0062 + current AI Reading Assist V2 + WordSenseOccurrence/ExamplePool + current Reader/TextBlockGroup | anti-duplicate prompt/preview contract 受测；sentence B matched existing 后进入现有 A/B/C 例句轮换且 retry/correction 不重复；AI 不生成虚构例句、绑定不写 ReviewLog/FSRS；strict paste-back 保持；有译文时 hidden/hover/show 几何一致、无译文时无空白占位；真实 Reader→Sense Review + 双 viewport |
