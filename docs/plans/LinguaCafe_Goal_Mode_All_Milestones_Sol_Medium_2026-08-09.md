...
| G-06B | TODO | 阅读机会式提前复习与单次计分：已学 exact Sense 在当前 due 尚未来到时也可以因真实阅读回忆提前记一次 Good；自然跳过可 passive Good，主动确认“认识/记得”可 explicit Good；existing Sense 明确“不认识”则 exact resolve 后最多一次 Again；同一 ReadingSession/ReviewCard 所有 occurrence 合并为最多一次正式 reading rating | ADR-0060 + existing ReadingSession/settlement + canonical ReviewCard/ReviewLog/FSRS writer | day-7 / prior day-30 forecast 的 early Good 可成立；同篇同 Sense 10 次最多 1 rating；求助不冒充 passive；unfamiliar→existing 同 session 后续 positive=0；new_sense 不伪造 lapse；跨 session 防刷不新建第二 scheduler/cooldown，先由 R2 Architecture Gate 冻结最小 predicate；真实 Reader + FSRS evidence |
