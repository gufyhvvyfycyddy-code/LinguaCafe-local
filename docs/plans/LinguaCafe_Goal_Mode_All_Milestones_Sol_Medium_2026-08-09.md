...
- Active milestone: `H-10 / E-08 remaining Apple capability cluster`; cloud macOS/Xcode/Simulator + rendered authenticated Keychain/session lifecycle + formal Sense Review Good/Undo + Reader touch/source-binding + real Files `.txt` import + offline/reconnect exactly-once sync are now runnable and Stage Accepted. The open boundary is now physical-device/signing/archive/TestFlight/App Store/App Review only.
...
- Current verified source/tooling baseline before this documentation closeout: pending current Goal closeout commit（manual/reusable iOS Xcode capability probe + checksum-pinned Maestro rendered smoke + overlapped Simulator cold boot + generated-Web integrity owner + checkout v6 runtime）.
- Last verified `origin/master`: `1c9bdcd74fa793356ba3938f21c56405f3261e39`（2026-08-30 fresh remote check）
- Deferred capability clusters: `iOS physical/distribution capability only — signed physical iPhone behavior, physical-device Keychain confirmation, physical haptics/notifications/audio/safe-area behavior, Apple signing/archive, TestFlight/App Store/App Review evidence. Simulator capability cluster is Stage Accepted; E-08 and H-10 remain DEFERRED / Not Complete only for this physical/distribution boundary.`
