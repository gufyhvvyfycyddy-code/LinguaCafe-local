...
> **最后更新**：2026-07-11 (ADR-0009: Review action transaction ledger + stack-based undo — Task B architecture complete. `ReviewCardFsrsSnapshotService` captures/restores 8 FSRS fields; `recordReview()` now wraps in `DB::transaction` + `lockForUpdate` capturing before/after snapshots; single additive migration adds 6 columns to `review_logs` (`review_session_id`, `before_card_snapshot`, `after_card_snapshot`, `undone_at`, `undo_request_id`, `undo_source`); `scopeNotUndone` excludes undone from all product analytics; `SenseReviewUndoPolicy` pure policy with 10 blocked reasons; `SenseReviewSessionActionService` read-only timeline; `SenseReviewUndoService` transactional undo with idempotency; 6 new backend test files (97 tests / 313 assertions); ADR-0007 deep link and ADR-0008 interval preview remain intact).
