...
- Phase 8G 只实现已冻结的 `missing:definition`、`missing:example`、`missing:source`。大小写不敏感，规范化为小写；相同 token 按首次出现去重，不同 token 使用既有全局 AND 语义。
- Criteria 新增有序只读 `missingFields`；Parser 继续负责纯函数解析、规范化和既有结构化 `invalid_browser_search` 422；前端只增加帮助、示例和简单 prefix 检测，没有复制完整 parser。
- 新增 `ReviewCardMissingFieldQueryApplier` 作为唯一缺字段谓词所有者；Browser token 与既有 `missing_definition`、`missing_example`、`missing_source` 顶部筛选均委托同一服务，避免 SQL 语义漂移。
- `missing:definition` 仅在 `sense_zh` 与 `sense_en` 都为 NULL 或空字符串时匹配；`missing:example` 只检查 `example_sentence_en`；`missing:source` 复用既有直接来源为空且无同用户、同语言、bound、chapter_id 非空 occurrence 的规则。
- 列表、JSON export、CSV export、Anki TSV export 继续共用同一 Parser、Criteria、Query Applier 与查询入口；没有新增 route、migration、dependency、AST、export field、Saved Search schema、Custom Study 或 Reviewer 行为。
- TDD RED：14 failed / 6 passed / 78 assertions；定向 GREEN：20 passed / 138 assertions / 0 failures；Browser 相关回归：420 passed / 1,523 assertions / 2 skipped / 0 failures；全部 JS guards 67/67，前端构建成功。
- 真实 localhost Chrome 输入 `MISSING:SOURCE` 后返回 HTTP 200，页面显示规范化 token `missing:source`；搜索动作只有一个本地 GET，无业务写请求，数据库动作前后计数一致。
- 完整 PHP 套件两轮均在接近结束时被测试进程实际固定的 128MB 内存上限中断；未出现功能断言失败，但没有声明 aggregate 通过。

## 0B. Phase 8E historical authority（2026-07-17）

Current phase: **Phase 3–7 authorized scope closed**；Phase 8A–8E 已按顺序完成。以下 Phase 8D 与更早章节保留历史证据，凡与本节冲突，以本节为准。

- 当前分支：`codex/anki-maximal-alignment-plan`；本轮预检时本地 HEAD、上游和远端均为 `fc291eff0e7d348c6f199a4432b3825e6a7420d5`。
- Phase 8E 只实现已冻结的 `source:chapter:<positive-id>` 与 `source:book:<positive-id>`。大小写不敏感，前导零规范化；不支持名称、自由文本、quoted phrase、`source:none` 或 `source:missing`。
- Criteria 新增来源目标；Parser 负责结构化 422、规范化和相同 token 去重；不同来源 token 使用现有全局 AND 语义。
- Query Applier 通过 SQL existence predicates 匹配 `WordSense.source_chapter_id` 或同用户、同语言、`status=bound` 的 `WordSenseOccurrence.chapter_id`；章节和书籍自身也必须属于当前用户和当前语言。
- 直接路径、occurrence 路径和多个 occurrence 不产生重复卡片；格式正确但不存在、已删除、跨用户或跨语言的 ID 返回零结果。
- Browser 的生命周期可见性保持不变：`filter=all` 时 suspended 和 archived sense card 仍可按来源找到；pending、rejected、ignored occurrence 不算真实来源。
- 列表、JSON export、CSV export、Anki TSV export 继续共用同一个 Parser、Criteria 和 Query Applier；没有新增 service、route、dependency、AST、migration 或导出字段。
- 本轮未保存可核验的 RED 运行记录，因此不宣称完成 RED→GREEN 证据链；完成态由 Parser、来源查询专项、既有 Browser Search 回归、全量 PHP、JS guards、构建和真实浏览器证据共同验证。
- 聚焦回归：150 passed / 600 assertions / 0 failures；补充跨用户、跨语言、无效 chapter/book ID 边界后 source 专项 5 passed / 35 assertions。
- 完整 PHP：3,293 tests / 13,428 assertions / 14 skipped / 64 既有 PHPUnit metadata deprecations / 0 failures（3m42.513s，exit 0）。
- 全部 JS guards：67/67 files passed；`npm run development` 编译成功，仅有既有 Sass deprecation warnings。
- 真实 localhost Chrome：在已登录的 `/review-cards/manage` 重新加载最新前端后，实际输入 `source:chapter:000999999` 并回车；HTTP 200，页面显示规范化 chip `source:chapter:999999`，无高级语法错误；帮助弹窗同时显示 `source:chapter:46`、`source:book:12` 和 AND 说明。
- Browser action Network：该搜索动作仅产生一个本地 GET `/review-cards/manage/data`；无 POST/PUT/PATCH/DELETE，无外部域名。Console 未出现来源搜索相关错误；页面重载时仍记录一条既有本地 Pusher WebSocket 连接失败信息，未将其误报为本轮新增错误。
- 数据库前后计数一致：ReviewLog 0、ReviewCard 4、WordSense 4、WordSenseOccurrence 0、review_card_state_events 0。
- 当前任务没有明确授权 commit/push，因此 Phase 8D/8E 相关文档和代码保持未提交；本地 HEAD、上游和远端仍为 `fc291eff0e7d348c6f199a4432b3825e6a7420d5`。
- 受保护的既有工作区内容保持原样：`.playwright-cli/`、`nul`、`public/js/app.js.LICENSE.txt`、`docs/architecture/improve-codebase-architecture-install-report.md`。

当前唯一下一小步：**Phase 8F — Browser Search missing-field token 只读侦察与语义冻结**。该步骤尚未授权执行；不得在本轮进入实现、recent windows、quoted phrase、negation、OR/parentheses 或其他后续阶段。

## 0B. Phase 8D historical authority（2026-07-17）

Current phase: **Phase 3–7 authorized scope closed**；Phase 8A、8B、8C 已按顺序完成，Phase 8D 已完成只读侦察与语义冻结。以下第 1–10 节保留早期 Phase 8B 交接证据，凡与本节冲突，以本节为准。

- 当前分支：`codex/anki-maximal-alignment-plan`；Phase 8D 预检时本地 HEAD、上游和远端均为 `fc291eff0e7d348c6f199a4432b3825e6a7420d5`。
- Phase 8D 只新增语义冻结文档 `docs/plans/review-card-browser-source-search-semantic-freeze.md`，并更新当前交接与 Phase 8 计划状态；没有修改业务代码、测试代码、路由、数据库、前端或运行时搜索语法。
- 冻结的首个实现语法仅为 `source:chapter:<positive-id>` 与 `source:book:<positive-id>`；不支持名称、自由文本、quoted phrase、`source:none` 或 `source:missing`。
- 来源成员关系冻结为：`WordSense.source_chapter_id` 直接路径，或同用户、同语言、`status=bound` 的 `WordSenseOccurrence.chapter_id` 路径；章节和书籍自身也必须属于当前用户与当前语言。
- 相同 token 去重；不同 `source:` token 使用现有全局 AND 语义。直接路径与 occurrence 路径、多个 occurrence 均不得产生重复卡片。
- 语法错误沿用结构化 `invalid_browser_search` 422；格式正确但不存在、已删除、跨用户或跨语言的 ID 返回零结果，不暴露其他范围是否存在该来源。
- `source:` 只表示真实章节/书籍溯源，不从保存例句、显示标签、章节名、书名、句子文本、导入来源、生命周期、FSRS 或 ReviewLog 推断。
- 未来实现继续使用现有 Parser、Criteria、Query Applier 和四个消费入口；只复用 Custom Study 已证明的来源成员关系语义，不直接复用其会排除部分生命周期状态的完整 eligible query。
- Phase 8D 为 docs/rules-only closure：真实浏览器验收不适用；必须通过相关规则守卫、本地链接检查和 `git diff --check`，并证明没有业务代码变化。
- 当前任务未明确授权 commit/push，因此 Phase 8D 文档变更保持未提交，不得借更新交接文档自行授权 Git 写入。
- 受保护的既有工作区内容保持原样：`.playwright-cli/`、`nul`、`public/js/app.js.LICENSE.txt`、`docs/architecture/improve-codebase-architecture-install-report.md`。
...
当前唯一下一小步：**Phase 8E — Browser Search `source:` token 最小实现**。该步骤尚未授权执行；本窗口在 Phase 8D 文档核验和交接完成后停止。
...
>
> **Latest direct execution — Preset V1D + ReaderSidebar-Boundary-Fix-1**
> - Real browser Preset matrix: main local account created `V1D Shared 20260715`, bound English, changed new-card limit to 21, switched French to the same Preset and read back 21, cloned/renamed/refreshed/deleted with safe rebind, then deleted the shared Preset and returned English/French to Default.
> - Second local admin account was created only for acceptance. Real logout/login confirmed it saw only its own Default Preset and could not see the main account's temporary Presets.
> - Main-account baseline and final facts were identical for learning data: `ReviewLog=166`, `ReviewCard=95`, due checksum `8efba9502402eab665070925ee0c6359bbb26f7164c3962e0262fb1defef76fb`.
> - Reader root cause: panel width and reserved track were the same, leaving about 0.67px boundary. The fix separates 540px panel width from 564px reservation width and removes the remaining duplicated breakpoint in `TextBlockGroup.vue`.
> - Chrome: 1920×1080 and 1780×861 showed about a 24.67px reader boundary and no horizontal overflow; 900×900 retained the 400px fallback, full sentence wrapping, and no overflow; Console was clean.
> - Local port 8000 became non-responsive during the final viewport pass while the process still listened. Only that hung development server process was restarted; no config, repository file, or database was changed by the restart.
...
| Browser / 管理页 | Card Info、Search、Table、Scheduling 与 Lifecycle 已形成职责所有者；Phase 3C-2 仍待真实认证页面收口 | `ReviewCardManage.vue` 1,210 行、11 axios、6 dialog；生命周期请求在管理页域内单一化；legacy `/enabled` 无可达表格入口，留待兼容清理 |
