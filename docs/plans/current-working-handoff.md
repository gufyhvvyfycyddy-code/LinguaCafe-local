...
> **最后更新**：2026-07-03 (GLM-ReadingInlineConfirmationPersistence-1000-1)
...
架构收口阶段已结束（总体架构收口 100%）。AI 示意卡 V1-V4 已实现。近期已新增多例句池、复习页题面例句轮换、答案后补充例句不重复、多来源溯源 carousel、Finished reading 确认弹窗、词形原型绑定（surface 保留 + lemma 优先搜索 + 用户修正后生效）、已学词义候选面板、熟词僻义前置结构（不调 AI）、例句池性能优化（消除 N+1 + 批量预加载）、真实 tokenizer/importer 端到端覆盖、8 类形态真实页面点击（MCP Chrome/Playwright 18 个真实点击）、词性歧义（published/used/broken/left）真实点击验证。本轮修复 OpenCode-RealClickFinalAudit-3 暴露的 morphology lemma 缺口：`technologies→technology` / `watches→watch` 等 PHP fallback `conservativeFallbackLemma()` 缺口已用 ECDICT-gated 可解释规则修复（`-ies → -y/-ie` + ultra-safe `-ches/-shes/-xes/-zes → strip -es`），新增 `MorphologyLemmaDefectFixTest` 6 个测试方法 / 149 断言，MCP Chrome 真实页面点击 15 次（13 OK + 2 P2 残留：`has broken→break` 与 `left side→left` 因 single-lemma-per-EncounteredWord 设计限制），未写 ReviewLog、未改 FSRS、未真实调用 AI、未实现阅读中刷卡评分、未实现 AI 判断熟词僻义。700% 是子阶段提升，不是固定五条主线虚假上涨。下一步仍应由网页端总设计师选择，不自动进入 AI 真实推荐、AI 释义生成或完整闭环，也不自动进入阅读中刷卡。
