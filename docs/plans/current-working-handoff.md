...
| 主线 | 进度 | 说明 |
|------|------|------|
| 总体架构收口 | ≈ 78% | FSRS/TextBlock/WordSense 各线 stabilization、测试、文档收口 |
| 文档与协作规则治理 | ≈ 94% | 已完成 Codex 交接文档、协作规则、本工作台，本轮完成后达 96% |
| sense-only 复习主线 | ≈ 85% | WordSense/ReviewCard/FSRS 核心链路已锁定，入口和 smoke 仍在进行 |
| SenseReview 真实页面 smoke harness | ≈ 90% | 到期卡评分已验收，pending occurrence/More 菜单路径待补 |
| AI 示意卡规划 | ≈ 10% | 产品目标已冻结，尚未进入架构侦查和实现 |
| 前端入口整理 | ≈ 50% | 已统一复习入口方向，尚未深入到 Vue 组件拆分 |
