...
| SenseSourceContext-QueryRenderExtract-1 | 拆分 source context 查询/渲染边界：新增 `SenseSourceContextResolverService`（查询/定位层），迁移 `resolveSense`/`findChapterById`/`findSourceSentenceKey`/`groupTokensBySentenceWithIndexes`/`contextEntriesAroundGroup`/`findMatchingChapterByExampleText`/`findMatchingChapterByFuzzyMatch`/`writeBackRecoveredSource`/`meaningfulTextTokens`/`targetTerms`/`fuzzySourceScore`/`collectTargetIndexes`。SenseSourceContextService 保持 public `sourceContext()` 门面，29 个测试全绿 + MCP Chrome API 200。不改 Vue、不改 Controller、不改 API shape、不改 source_kind/fallback 顺序/写回条件。 |
