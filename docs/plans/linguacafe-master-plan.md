...
4. **Queue Order** — 架构研究完成，开发未开始（Task `Anki-Queue-Order-Architecture-Research-2000-9A` docs-only: ADR-0015 + `docs/plans/review-queue-order-implementation-plan.md` 已完成; 推荐 Approach A 学习优先 5 buckets (relearning → learning → overdue_review → today_review → new); `Anki-Queue-Order-Development-2000-9B` 待开发; 本轮不改业务代码, 不改测试, 不改排序, 不改 FSRS/lifecycle/ReviewLog, 不新增 migration/setting; `ReviewService::shuffle()` 与 `Review.vue` `Math.random()` 现状已记录但本轮未删除）
5. **Custom Study 1A** — 自定义学习会话第一版
6. **Saved Search** — 保存搜索
7. **today-only limits** — 今日限制
8. **Study Overview** — 学习概览

不提前把 Queue Order、Custom Study、Saved Search、today-only limits、Study Overview 或其他后续任务标记完成。Card Info (Task 2000-8) 已完成。Queue Order 架构研究 (Task 2000-9A) 已完成 docs-only，开发 (Task 2000-9B) 未开始。
