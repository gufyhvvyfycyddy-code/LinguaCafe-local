...
> | Current Phase | Phase 6A–6F are Accepted / Production Closed; Reader product small steps are complete and remaining Phase 6 structural governance is current |
...
| 6 | Reader UI / architecture governance | Current | Phase 6A–6F 已 Accepted / Production Closed；产品小步完成，下一切片继续一次治理一个已表征结构职责 | 不改变阅读主链路、tokenizer、surface/lemma、WordSense、ReviewCard、ReviewLog 或 FSRS | 同路线 Phase 6 | scoped architecture gate → harness → implementation → browser acceptance |
| 7 | Reader architecture governance | Current | 降低阅读页改一处坏多处的风险 | `TextBlockGroup.vue` 2163 行；`TextBlockService.php` 1382 行；需逐职责 characterization + browser acceptance | 同路线 Phase 6；hotspot audit | Reader UI 小步已完成，阶段预授权生效 |
