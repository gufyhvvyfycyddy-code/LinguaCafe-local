...
> **最后更新**：2026-07-14 (Task 2000-17 `GLM-Hard-Rules-Canonicalization-And-CustomStudy-Phase2A-Query-Foundation-2000-17` — **复合型主线任务**。本轮完成：(A) 长期硬规则治理：在 `vibe-coding-collaboration-rules.md` §28 写入"需求放置、复合任务与报告闭环硬规则"唯一权威正文；§19 复杂度规则收敛为唯一权威（§8.8 缩减为短引用，100 是最低基线非上限）；(B) Phase 1 错误契约架构修复：`CustomStudyCriteria` 直接抛出结构化 `CustomStudyValidationException`，删除 Validator 的 message parsing；(C) Phase 2A 只读查询：`TodayForgottenQuery` + `OverdueQuery` 返回可组合 Builder。**Custom Study 1A 当前状态**：Phase 1 功能通过 + 错误契约修复完成待验收；Phase 2A 完成待验收；Phase 2B-6 未开始；整体功能不可使用。**Queue Order 状态**：✅ Accepted / 生产验收通过（Task 2000-14）。**硬规则权威位置**：`vibe-coding-collaboration-rules.md` §28 需求放置/复合任务/报告闭环；§19 复杂度规则。本文件不复制全文，仅引用。当前工作流: GLM 单 Agent 闭环, 详见 `vibe-coding-collaboration-rules.md` §1.5; CodeBuddy/WorkBuddy 已停用, 旧规则见 `docs/history/codebuddy-workbuddy-workflow-archive-2026-07-13.md`)。
