...
> | Current Phase | Goal Phase G + G-GATE、pre-H `H-00`、`H-01` load/observability 与 `H-02` representative 100-user load 均已 DONE；当前 forward milestone 为 `H-03` bottleneck-only performance repair，第一步是分解 login/setup 与 Reading / lookup / Sense Review latency，并关联 MySQL statement digests 与 Apache concurrency。 |
...
> **最后更新**：2026-08-28。恢复发布程序历史已关闭；Goal Phase G + G-GATE、pre-H H-00、H-01 load/observability 与 H-02 representative 100-user load 已完成。H-02 exact implementation commit 为 `bc2cb433101751e8c6922d8cc803137988bc0657`，验收见 `docs/testing/h02-representative-load-acceptance-2026-08-28.md`；100 VU correctness gate 全绿，但 aggregate p95≈6.87s。当前 active milestone 为 H-03：先分解 login/setup、Reading、lookup、Sense Review 的 latency 与 MySQL/Apache pressure，再只对实测瓶颈付出 query/batch/index/cache/runtime tuning 成本。Open Work Registry 继续保存运维/外部能力账本，但不再冒充唯一产品待办。Reader forward 由 ADR-0061 收束为“满 24h 后可机会式提前 Good / 不认识=Again / 同 session 单次计分”，ADR-0063 补充“24h 不足时普通 Reader 完全静默”；ADR-0060/0059 只保留历史。AI `matched_existing` 的真实来源句子进入现有例句池由 ADR-0062 控制，全部真实来源例句无 top-N 上限、完整池随机且不连续重复由 ADR-0064 控制。
