...
> **最后更新**：2026-07-13 (ADR-0012 Task 2000-6 fix: Browser Search export 422 contract — `exportAnkiTsv()`/`exportCsv()` return type 从 `\Illuminate\Http\Response` 改为 `\Symfony\Component\HttpFoundation\Response` (公共父类型), 修复非法语法和导出超限时返回 JsonResponse 导致的 PHP TypeError; 6 新增测试 CSV/TSV 422 + 合法导出 200 + 不写 ReviewLog + 不改 FSRS/lifecycle; MCP Chrome 13/13 PASS; 不改 parser 管道, 不改 FSRS/lifecycle/ReviewLog/ADR-0010/ADR-0011; 后续顺序: Browser Search fix → Search architecture convergence → Card Info → Queue Order → Custom Study 1A → Saved Search → today-only limits → Study Overview).
...
2. **Search architecture convergence** — `Anki-Browser-Search-Architecture-1-2000-7`（重复解析问题收敛、parser 管道架构优化，留作下一步架构任务）
3. **Card Info** — 卡片详情面板
4. **Queue Order** — 新卡 / 学习中 / 到期卡的队列顺序与积压优先级
5. **Custom Study 1A** — 自定义学习会话第一版
6. **Saved Search** — 保存搜索
7. **today-only limits** — 今日限制
8. **Study Overview** — 学习概览

不提前把架构任务、Card Info 或其他后续任务标记完成。
