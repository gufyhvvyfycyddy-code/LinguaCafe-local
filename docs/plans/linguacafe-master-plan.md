...
> **Custom Study 1A update (2026-07-14)**: Phase 5A data-contract work is underway. The authenticated read-only `GET /custom-study/chapter-options` endpoint returns stable user/language-scoped eligible chapters with full distinct candidate counts; the three POST session routes stay unchanged. Selected example sentence, token payload, and persisted translation are aligned. No frontend page/session UI, migration, ReviewLog, FSRS, lifecycle, or AI-provider behavior is included in this slice.
