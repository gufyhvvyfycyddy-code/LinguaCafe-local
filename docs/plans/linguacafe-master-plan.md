...
> | Current Phase | Phase 6A–6K are Accepted / Production Closed; Reader product small steps and lookup-response policy extraction are complete, and lookup HTTP transport is the remaining Phase 6 frontend target |
...
>
> **Architecture status correction**: the old “overall architecture closure 100%” statement is historical. Settings convergence and Browser ownership extraction removed major concentrated debt. The current assessment remains **6.0/10, localized medium-high burden**. `ReviewCardManage.vue` is 668 lines with 4 coordinator requests and no dialogs; Phase 6A–6E reduced `TextBlockGroup.vue` from 2,514 to 2,163 lines through tested pure policies, Phase 6F closed the Reader action-hierarchy/focus product step, Phase 6G reduced the component to 2,099 lines by extracting hotkey intent, Phase 6H reduced it to 2,041 lines by extracting navigation candidate selection, Phase 6I reduced it to 2,035 lines by extracting completion-candidate classification, Phase 6J reduced it to 2,033 lines while moving seven token-presentation rules without changing DOM or events, and Phase 6K reduced it to 2,000 lines by extracting lookup-response normalization without moving transport or effects. Lookup HTTP transport remains the active Reader target.
>
...
| 6 | Reader UI / architecture governance | Current | Phase 6A–6J 已 Accepted / Production Closed；产品小步、selection/keyboard/hover/completion/token-presentation 纯规则已有 owner，最后治理 lookup orchestration | 不改变阅读主链路、tokenizer、surface/lemma、WordSense、ReviewCard、ReviewLog 或 FSRS | 同路线 Phase 6 | scoped architecture gate → harness → implementation → browser acceptance |
| 7 | Reader architecture governance | Current | 降低阅读页改一处坏多处的风险 | `TextBlockGroup.vue` 2033 行；`TextBlockService.php` 1382 行；lookup orchestration 需 characterization + browser acceptance | 同路线 Phase 6；hotspot audit | Reader UI 小步已完成，阶段预授权生效 |
