...
> **最后更新**：2026-07-14 (Task 2000-19 `GLM-CustomStudy-Phase3A-SessionState-And-EncryptedToken-Foundation-2000-19` — **Custom Study 1A Phase 3A 复合型主线任务**。本轮完成：(A) 正式关闭 Phase 2B 文档状态（Phase 1/2A/2B 统一 Accepted；ADR-0016 路径修正 `app/CustomStudy/` → `app/Services/CustomStudy/`；权威文件清单与测试清单补齐 Phase 1/2A/2B/3A 既有文件）。(B) 修复 Session State 与 Token payload 架构缺口：V1 payload 新增 `completed_ids` + `skipped_ineligible_ids`；五状态 union + 互斥不变量从 state 单独可验证；`completed_count === count(completed_ids)`；`total_count === count(ordered_candidate_ids)`；时间字段统一 UTC Unix seconds；`session_id` 严格 UUID v4；`step` 为非负整数 token revision。(C) 不可变 `CustomStudySessionState`（值对象，无 DB/Auth/Request/Crypt/ReviewLog/FSRS/AI；`createInitial()` + `fromArray()` + `toArray()` + 只读 getter；无 setter；无 answer/rate/resume/nextCard/transition/rotate）。(D) `CustomStudySessionTokenService`（只加解密 + 验证；注入 `Illuminate\Contracts\Encryption\Encrypter`；`issue()` + `verify()`；无 `rotate(answer)`；`MAX_TOKEN_BYTES=65536`、`DEFAULT_TTL_SECONDS=14400`、`MAX_CANDIDATE_COUNT=500`）。(E) 章节选择器未来需求登记到 ADR-0016 §21（只登记，不实现；冻结 Phase 5/6 批量查询契约）。(F) TDD + 完整回归 + FACT 自审 + 3 commits。**Previous task (2000-18)**: Phase 2B `EloquentChapterLocator` + `SourceChapterQuery` + `LeechAttentionQuery` + `CustomStudyQueryService` + 86 行为测试；Phase 2A 文档旧契约修正。**Custom Study 1A 当前状态**：Phase 1 ✅ Accepted；Phase 2A ✅ Accepted；Phase 2B ✅ Accepted（Task 2000-18）；Phase 3A（`CustomStudySessionState` + `CustomStudySessionTokenService` + `CustomStudySessionStateException` + 2 unit tests）代码与测试完成，等待网页端验收；Phase 3B / Phase 4-7 未开始；**整体功能不可使用**。**Queue Order 状态**：✅ Accepted / 生产验收通过（Task 2000-14）。**硬规则权威位置**：`vibe-coding-collaboration-rules.md` §28 需求放置/复合任务/报告闭环；§19 复杂度规则。本文件不复制全文，仅引用。当前工作流: GLM 单 Agent 闭环, 详见 `vibe-coding-collaboration-rules.md` §1.5; CodeBuddy/WorkBuddy 已停用, 旧规则见 `docs/history/codebuddy-workbuddy-workflow-archive-2026-07-13.md`)。
