...
> **最后更新**：2026-07-13 (ADR-0013 Task 2000-7 refactor: Review Card Browser Search execution pipeline convergence — 每请求只 parse 一次, `buildFromCriteria()` 单一执行入口, 旧 `build()` 降级为薄包装, 四入口共用 Criteria, Parser 按首次出现顺序去重 (`normalizedTokens` / `ratings` / `propertyConditions`), 同类冲突仍 422, governance 单次批量 O(1), 不复制 Leech Policy, 不逐卡查 ReviewLog, 不改 V1 结果, 不新增语法; 6 parser 去重 + 16 ADR-0013 pipeline feature 测试, 回归套件 649 passed / 0 failed; npm build 成功; db:doctor HEALTHY; MCP Chrome 18/18 PASS; 不改 FSRS / lifecycle / ReviewLog / ADR-0010 / ADR-0011; 后续顺序: Card Info → Queue Order → Custom Study 1A → Saved Search → today-only limits → Study Overview).
