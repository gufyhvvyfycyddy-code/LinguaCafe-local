...
> | Current Phase | Goal Phase G + G-GATE、pre-H `H-00`、`H-01` load/observability、`H-02` representative 100-user load 与 `H-03` bottleneck diagnostics 均已 DONE；当前 forward milestone 为 `H-04` testing-only backup/restore drill，复用既有 M6 Backup/Restore owners。 |
...
> **最后更新**：2026-08-28。恢复发布程序历史已关闭；Goal Phase G + G-GATE、pre-H H-00、H-01 load/observability、H-02 representative load 与 H-03 bottleneck diagnostics 已完成。H-03 exact instrumentation commit 为 `a3859e2158a926153a5d8f7a27d3565014307fca`，验收见 `docs/testing/h03-bottleneck-diagnostics-acceptance-2026-08-28.md`；100 VU Reading/lookup/Sense Review p95≈36.3/23.1/48.8ms，aggregate 秒级 p95 由 fresh Apache prefork cold-burst GET `/login` 主导，因此没有证据支持改写业务 query/index/cache/FSRS/session。当前 active milestone 为 H-04：只在隔离 testing DB 做既有 Backup/Restore owner 的真实恢复演练。Open Work Registry 继续保存运维/外部能力账本，但不再冒充唯一产品待办。Reader forward 由 ADR-0061 收束为“满 24h 后可机会式提前 Good / 不认识=Again / 同 session 单次计分”，ADR-0063 补充“24h 不足时普通 Reader 完全静默”；ADR-0060/0059 只保留历史。AI `matched_existing` 的真实来源句子进入现有例句池由 ADR-0062 控制，全部真实来源例句无 top-N 上限、完整池随机且不连续重复由 ADR-0064 控制。
