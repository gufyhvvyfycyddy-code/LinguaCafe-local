...
> | State | Work |
> |---|---|
> | Production Closed | Queue Order; Custom Study 1A; Manual Sense POS + shared form; Settings architecture convergence; Preset V1A |
> | Accepted | Saved Search V1; Mgmt-7-b; today-only limits; Review Time; Study Overview canonical eligibility |
> | Current Next Task | Preset V1B — Management Operations and UI |
> | Authorized Sequence | Preset V1C consumer convergence → Preset V1D UX/production closure → Browser/ReviewCardManage convergence → Card Marker + Custom Study 1B → Reviewer convergence → Reader UI/reader architecture |
...
| 2B | Preset V1B — Management Operations and UI | Planned / Current Next Task | 新增、复制、重命名、删除、切换 Preset；设置页成为当前 Preset 编辑器 | Default 不可删除/重命名；删除普通 Preset 必须事务重绑定；所有权、名称冲突和共享影响必须可见 | `review-settings-preset-v1-plan.md` | 已授权为下一任务 |
| 2C | Preset V1C — Consumer Convergence | Planned | 所有设置消费者只读取当前 binding；清除多用户环境下的残留全局权威 | `fsrs_parameters_previous` 仍写入全局 Setting 且没有真实读取方；需决定删除、移入 Preset 操作历史或明确废弃 | `review-settings-preset-v1-plan.md`；ADR-0024 | V1B 关闭后授权 |
| 2D | Preset V1D — Settings UX and Production Closure | Planned | 管理动作、空状态、危险操作和多语言共享形成完整稳定体验 | 高级工具在无/少数据状态信息重复、零值卡片过多、无效动作仍突出；需完成 Settings UX-1 后再最终关闭 | `review-settings-preset-v1-plan.md`；截图反馈 2026-07-15 | V1C 关闭后授权 |
...
| 2B | Preset V1B | Current Next Task | 新增/复制/重命名/删除/切换；Default 保护、所有权和删除重绑定必须事务化 |
| 2C | Preset V1C | V1B 后续 | 全消费者收敛，并处理 `fsrs_parameters_previous` 全局孤儿状态 |
| 2D | Preset V1D | V1C 后续 | Settings UX-1 空状态/动作安全收口与最终生产验收 |
