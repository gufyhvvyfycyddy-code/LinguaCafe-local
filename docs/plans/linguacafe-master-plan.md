...
| Anki-Leech-1 | 遗忘卡识别：按 lapses 阈值标记，默认提示用户改写释义卡；是否自动暂停由用户设置决定 | ✅ 已完成 (commit 9a03740 + Task A, ADR-0011, stable/struggling/leech classification from ReviewLog + pure policy + batch query no N+1 + rewrite package provider_called=false + 4 endpoints + 56 backend tests + 94 frontend guard tests + leech NOT lifecycle state + no migration) |
...
下一主线为 `Anki-Browser-Search-1`（高级浏览器搜索语法：is:leech、is:suspended、rated:again、prop:lapses），不自动进入。
