...
> | Current Phase | Goal Phase G + G-GATE、pre-H `H-00`、`H-01` load/observability、`H-02` representative 100-user load、`H-03` bottleneck diagnostics 与 `H-04` backup/restore drill 均已 DONE；当前 forward milestone 为 `H-05` isolation/privacy boundary。 |
...
> **最后更新**：2026-08-28。恢复发布程序历史已关闭；Goal Phase G + G-GATE、pre-H H-00、H-01 load/observability、H-02 representative load、H-03 bottleneck diagnostics 与 H-04 backup/restore drill 已完成。H-04 exact implementation commit 为 `e2cfc4427fb49dd2806ad4d105100164296e998d`，验收见 `docs/testing/h04-backup-restore-drill-acceptance-2026-08-28.md`；真实 Oracle MySQL 8 backup→restore、write fence、automatic safety rollback 与 zero-residue 均通过，专用 restore identity 使用 Performance Schema 做 fail-closed quiescence，Web DB 用户无需全局 PROCESS。当前 active milestone 为 H-05 isolation/privacy boundary。Open Work Registry 继续保存运维/外部能力账本，但不再冒充唯一产品待办。Reader forward 由 ADR-0061 收束为“满 24h 后可机会式提前 Good / 不认识=Again / 同 session 单次计分”，ADR-0063 补充“24h 不足时普通 Reader 完全静默”；ADR-0060/0059 只保留历史。AI `matched_existing` 的真实来源句子进入现有例句池由 ADR-0062 控制，全部真实来源例句无 top-N 上限、完整池随机且不连续重复由 ADR-0064 控制。
