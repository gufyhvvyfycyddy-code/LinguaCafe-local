...
> | Current Phase | Phase 6A hover lookup and Phase 6B hover position policies are Accepted / Production Closed; remaining Phase 6 Reader UI and architecture governance is current |
...
| 6 | Reader UI / architecture governance | Current | Phase 6A hover lookup 与 Phase 6B hover position policy 已 Accepted / Production Closed；下一切片继续一次治理一个已表征职责 | 不改变阅读主链路、surface/lemma、WordSense、ReviewCard、ReviewLog 或 FSRS | 同路线 Phase 6 | scoped architecture gate → harness → implementation → browser acceptance |
| 7 | Reader architecture governance | Planned | 降低阅读页改一处坏多处的风险 | `TextBlockGroup.vue` 2452 行；`TextBlockService.php` 1382 行；需逐职责 characterization + browser acceptance | 同路线 Phase 6；hotspot audit | Reader UI 小步后授权 |
