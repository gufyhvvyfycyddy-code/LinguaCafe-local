...
5. **Custom Study 1A** — 自定义学习会话第一版（架构契约已冻结：ADR-0016 + `custom-study-1a-implementation-plan.md`；**Phase 1 ✅ Accepted；Phase 2A ✅ Accepted；Phase 2B ✅ Accepted（Task 2000-18：`EloquentChapterLocator` + `SourceChapterQuery` + `LeechAttentionQuery` + `CustomStudyQueryService`，14+27+27+18=86 tests）；Phase 3A ✅ Accepted（Task 2000-19 + Task 2000-20 docs closure：`CustomStudySessionState` 不可变值对象 + `CustomStudySessionTokenService` 加解密+验证 + `CustomStudySessionStateException` + 2 unit tests）；Phase 3B 代码与测试完成（Task 2000-20：`CustomStudyPreviewPolicy` 纯函数 + `withProgress` 不可变边界 + `waitUntil` + `isCompleted` + `CustomStudyPreviewPolicyException` + 51 tests），文档/harness 状态漂移已收口（Task 2000-21），等待网页端最终关闭；Phase 4A SessionOrder 代码与测试完成（Task 2000-21：`CustomStudySessionOrder` 会话内部排序服务 + 55+ tests），等待网页端验收；Phase 4B-7 未开始**，需网页端总流程设计师单独授权才能进入。授权后建议开发顺序：Phase 1 值对象/校验器（已实现） → Phase 2 查询服务（2A+2B 已实现） → Phase 3A Session State + Token Service（已实现） → Phase 3B PreviewPolicy + withProgress（已实现，待最终关闭） → Phase 4A SessionOrder（已实现，待验收） → Phase 4B SessionService（会话编排：validate token → re-validate eligibility → apply PreviewPolicy → pick next → issue refreshed token） → Phase 5 前端（包含 **共享卡面抽取阶段**：`Task CS-11.5: Shared card presentation component SenseStudyCard.vue`，落位 ADR-0016 §20，含阅读式 token 句子展示 + 空答案字段隐藏契约 + AI 译文卡面需求登记 + 章节选择器只显示存在候选卡的章节契约 ADR-0016 §21，含 Task 2000-21 登记的 `candidate_count` MUST display + `candidate_count = 0` 禁止显示 + candidate_count 语义 OPEN PRODUCT GATE） → Phase 6 路由集成 → Phase 7 端到端 MCP Chrome 验收。SenseStudyCard 抽取阶段必须复用 `SenseSentencePreview.vue`，禁止 v-html，禁止 tokenizer 重调用，禁止新增 API/migration。**整体功能尚不可使用**。）
6. **Saved Search** — 保存搜索
7. **today-only limits** — 今日限制
8. **Study Overview** — 学习概览

不提前把 Custom Study、Saved Search、today-only limits、Study Overview 或其他后续任务标记完成。Card Info (Task 2000-8) 已完成。Queue Order (Task 2000-9A 架构研究 + Task 2000-9B 架构修正+开发 + Task 2000-10A 生产收口 + Task 2000-13 recovery helper + Task 2000-14 lock fix 最终关闭) ✅ Accepted / 生产验收通过（Task 2000-14，网页端最终验收，2026-07-14）。Custom Study 1A 架构契约已冻结（ADR-0016 + implementation plan）；Phase 1 ✅ Accepted；Phase 2A ✅ Accepted；Phase 2B ✅ Accepted（Task 2000-18：`EloquentChapterLocator` + `SourceChapterQuery` + `LeechAttentionQuery` + `CustomStudyQueryService`，86 tests）；Phase 3A ✅ Accepted（Task 2000-19 + Task 2000-20 docs closure：`CustomStudySessionState` + `CustomStudySessionTokenService` + `CustomStudySessionStateException` + 2 unit tests）；Phase 3B 代码与测试完成（Task 2000-20：`CustomStudyPreviewPolicy` + `withProgress` + `waitUntil` + `isCompleted` + 51 tests），文档/harness 修复完成（Task 2000-21），等待网页端最终关闭；Phase 4A SessionOrder 代码与测试完成（Task 2000-21：`CustomStudySessionOrder` + 55+ tests），等待网页端验收；Phase 4B-7 未开始，需网页端总流程设计师单独授权才能进入。
