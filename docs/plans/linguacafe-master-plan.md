...
> | Current Phase | Browser/ReviewCardManage Phase 3A — Card Info Drawer Extraction is Implemented / Automated Verified / Authenticated Network Acceptance Blocked / Incomplete; Phase 3B is not authorized |
...
| 3 | Browser / ReviewCardManage architecture convergence | Phase 3A Implemented / Automated Verified / Authenticated Network Acceptance Blocked / Incomplete | Anki Browser 式搜索、表格、Card Info 与危险操作分区；Phase 3A 已抽取只读 Card Info Drawer | `ReviewCardManage.vue` 2,792 行、22 个 direct `axios.` references、12 个 `v-dialog`；Card Info canonical detail request 仅由子组件负责 | `docs/plans/review-card-manage-architecture-convergence-plan.md`；ADR-0012–0014、0017 | 登录态恢复后完成真实页面 Network trace 与最终验收；不得进入 Phase 3B 或 Card Marker |
...
| 3 | Browser / ReviewCardManage convergence | Phase 3A Implemented / Automated Verified / Authenticated Network Acceptance Blocked / Incomplete | `ReviewCardManage.vue` 已完成 Card Info Drawer Extraction；当前无法取得登录后的真实 Network trace，Phase 3B 未授权；见 `review-card-manage-architecture-convergence-plan.md` |
