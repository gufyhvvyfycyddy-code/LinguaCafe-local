...
> | Current Phase | No implementation phase is currently authorized; Browser/ReviewCardManage Phase 3C-3 is Accepted / Production Closed; Phase 3C-4 Leech Governance Mutation Family is Planned / Not Authorized |
...
| 3 | Browser / ReviewCardManage architecture convergence | Completed / Production Closed through Phase 3C-3 | Phase 3A 抽取只读 Card Info；Phase 3B-1 抽取 Search；Phase 3B-2 抽取 Table；Phase 3C-1 抽取 Due-now / Reset；Phase 3C-2 抽取 Lifecycle Mutation Family；Phase 3C-3 抽取 Delete Mutation Family | `ReviewCardManage.vue` 1,098 行、9 个 direct `axios.` references、4 个 `v-dialog`；`ReviewCardDeleteMutationSurface.vue` 196 行、1 个 DELETE、1 个 bulk-delete POST、2 个确认弹窗；表格继续只拥有当前行、选择和 intent；父组件仍有无可达表格入口的 legacy `/enabled` 兼容代码 | `docs/plans/review-card-manage-architecture-convergence-plan.md`；`docs/testing/review-card-delete-mutation-browser-acceptance-2026-07-17.md`；自动回归、构建与 authenticated browser 双 viewport 验收完成 | Phase 3C-4 Leech Governance Mutation Family Planned / Not Authorized；需要单独授权 |
...
| 3 | Browser / ReviewCardManage convergence | Phase 3C-3 Accepted / Production Closed | Card Info、Search、Table、Due-now / Reset、Lifecycle 与 Delete Mutation Surface 已形成独立职责所有者；父组件 1,098 行、9 个请求、4 个弹窗；Phase 3C-4 Leech Governance Mutation Family Planned / Not Authorized；见 `review-card-manage-architecture-convergence-plan.md` |
