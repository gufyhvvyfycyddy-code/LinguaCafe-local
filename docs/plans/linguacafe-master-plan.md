...
> | Current Phase | Phase 6A hover lookup, Phase 6B hover position, Phase 6C sentence context, Phase 6D drag selection, and Phase 6E phrase instance selection policies are Accepted / Production Closed; remaining Phase 6 Reader UI and architecture governance is current |
...
| 6 | Reader UI / architecture governance | Current | Phase 6A hover lookup、Phase 6B hover position、Phase 6C sentence context、Phase 6D drag selection 与 Phase 6E phrase instance selection policy 已 Accepted / Production Closed；下一切片继续一次治理一个已表征职责 | 不改变阅读主链路、tokenizer、surface/lemma、WordSense、ReviewCard、ReviewLog 或 FSRS | 同路线 Phase 6 | scoped architecture gate → harness → implementation → browser acceptance |
| 7 | Reader architecture governance | Planned | 降低阅读页改一处坏多处的风险 | `TextBlockGroup.vue` 2163 行；`TextBlockService.php` 1382 行；需逐职责 characterization + browser acceptance | 同路线 Phase 6；hotspot audit | Reader UI 小步后授权 |
