...
> | Current Phase | Goal Phase G + G-GATE、`H-00`–`H-09`、`H-11` 均已 DONE；2026-08-30 已补齐 E-06/E-07 Android 原生能力，并通过 public GitHub `macos-26` runner 真实证明 H-10 的 Xcode 26.6/SwiftPM/iOS Simulator compile+launch、rendered login-shell、same-runner testing backend 下的 authenticated login→Keychain save/load→relaunch→server token owner→UI revoke→token clear 生命周期、run `33282205923` 的正式 Sense Review Good→下一卡→Undo→exact FSRS restoration，以及 run `33301226295` 的真实 Reader token/Sense/source binding 与横屏词组长按拖选。run `33308079898` 已关闭真实 Files `.txt` import，final full run `33350591521` 又关闭 offline article/review/audio cache、离线 Good 入队、relaunch persistence、reconnect exactly-once sync 与 strict cleanup，因此 H-10 Simulator capability cluster 已 Stage Accepted。`H-GATE` 仍为 DEFERRED / Not Complete，因为 `E-08` / `H-10` 仍缺 signed physical device、physical Keychain/haptics/notifications/audio/safe-area、Apple signing/archive、TestFlight/App Store/App Review capability。 |
...
>
> **Architecture status**: current measurements and hotspot evidence live in `docs/CURRENT_AI_CONTEXT.md` and the architecture audits. This authority block does not freeze file line counts or a permanent numeric debt score.
>
...
> **最后更新**：2026-08-31。Goal Phase G + G-GATE、H-00–H-09 与 H-11 已完成；E-06/E-07 的历史 Android capability gap 已由 Android 16 真实模拟器证据关闭。H-GATE final audit 已执行并保持 DEFERRED / Not Complete。Windows 本机仍无 Xcode/Tailscale Mac，但标准 GitHub-hosted `macos-26` capability lane 已真实通过 Xcode 26.6、SwiftPM、Simulator build、iPhone 17 Pro Simulator boot/install/launch、rendered login-shell Accessibility/用户输入 smoke，以及 same-runner testing backend 下的 authenticated login/Keychain/relaunch/revoke 生命周期；cold boot 也已与 build 重叠。正式 Sense Review Good/Undo 已由 run `33282205923` 关闭，Reader touch/source-binding 已由 run `33301226295` 关闭，Files `.txt` import 已由 run `33308079898` 关闭，offline/reconnect full matrix 已由 run `33350591521` 关闭。Simulator capability cluster 现已 Stage Accepted；2026-08-31 release-capability run `33355499203` 又验证 Xcode 26.6 `arm64` capability、iPhone 17 Pro + `iPad Pro 13-inch (M5)` 串行 rendered shell，以及 `2064x2752` 13-inch iPad screenshot。当前剩余 iOS blocker 只在 signed physical iPhone、physical Keychain/haptics/notifications/audio/safe-area、Apple signing/archive、TestFlight/App Store/App Review、public Privacy/Support URL。
...
## 4. Open Work Registry（当前运行、维护与外部能力账本）

> 本节只保留尚未完成、仍需外部能力或需要新授权的运行/维护事项。已关闭里程碑只在对应 ADR、验收报告和历史段落中保留，不再重复列入当前待办。已冻结的产品决定由产品决定文档单独持有，不因为“可进入 Architecture Spec”而成为当前 active task。

| 顺序 | 事项 | 当前状态 | 下一步 / 停止条件 | 权威证据 |
|---:|---|---|---|---|
| 1 | M9 iOS sync、编译、签名、设备、TestFlight 与 App Store 证据 | `Deferred / Not Complete` | 需要 macOS、Xcode、Apple 签名身份和真实设备/商店能力；当前 ignored iOS generated public 仍为旧 bundle 且含 sourcemap，必须先受控 sync 并通过资源完整性门禁；不得在 Windows 上伪造完成 | ADR-0052、ADR-0054、M9 acceptance/playbook |
| 2 | Runtime AI provider 激活 | `Environment Gate / Explicit Authorization Required` | 只有用户明确批准 provider、模型、密钥、外发、成本和超时后才能启用；default-off 保持关闭 | ADR-0004、ADR-0005、ADR-0030 |
| 3 | 本地体验与 Bug 维护账本 | `Planned / Not Yet Authorized` | 按 P0/P1 顺序一次选择一个可验证切片；不得把维护任务扩成新产品开发 | `local-experience-bug-optimization-ledger-2026-07-23.md` |
...
当前已确认产品方向与待讨论边界以 `docs/product/confirmed-product-decisions-and-discussion-roadmap-2026-07-23.md` 为准。

本文档适用于网页端 GPT、网页端 DevSpace 和本地 Codex：
...
- **MCP Chrome 真实页面双 viewport 验收**（GLM 自己执行，未用 API 200 代替）：
  - 宽屏 1920x900：登录 `当前任务提示词提供的本地测试账号` → `/reviews/senses` 显示 22 张到期 sense card → 筛选出 3 张多例句卡（`occurrence_count=2`）：`codex_sense_smoke_20260702_b_bind_target` / `codex_sense_smoke_20260702_b_confirm_target` / `codexmatrix` → 多例句提示 chip 正确显示「本词义已有 2 条来源例句」 → 显示答案 → 答案侧完整 → 评分 `good` → 跳到下一张多例句卡，提示 chip 同样正确 → 查看原文/译文对话框打开（sourceCount=1，未跟随 occurrence_count=2 → **P2 记录**） → 轮换验证（临时 PHP 脚本对 card#61 reps 0-10 序列化，产生 2 个不同 `displayed_occurrence_id`，DB 未写入）。Network 全部 127.0.0.1:8000，Console 仅 WebSocket 降级。
...
- 登录 `当前任务提示词提供的本地测试账号` → `/reviews/senses`，复习 5 张卡（published / codexmatrix / technology / city / window）
