...
> | State | Work |
> |---|---|
> | Production Closed | Queue Order; Custom Study 1A; Manual Sense POS + shared form; Settings architecture convergence; Preset V1A; Preset V1B; Preset V1C; Preset V1D; Browser/ReviewCardManage Phase 3A; Browser/ReviewCardManage Phase 3B-1; Browser/ReviewCardManage Phase 3B-2; Browser/ReviewCardManage Phase 3C-1 |
...
> | Current Phase | Browser/ReviewCardManage Phase 3C-2 — Lifecycle Mutation Family is Code Complete / Browser Acceptance Pending; Phase 3C-3 Delete Mutation Family is Planned / Not Authorized |
...
>
> **Architecture status correction**: the old “overall architecture closure 100%” statement is historical. Settings convergence removed two former 1,000-line hotspots. The fresh baseline is 28 production files over 500 lines, 10 over 1,000, and 2 over 1,500; current assessment remains **6.5/10, localized high burden** because `ReviewCardManage.vue`, `TextBlockGroup.vue`, and `SenseReview.vue` remain large.
>
...
| 3 | Browser / ReviewCardManage architecture convergence | Phase 3C-2 Code Complete / Acceptance Pending | Phase 3A 抽取只读 Card Info；Phase 3B-1 抽取 Search；Phase 3B-2 抽取 Table；Phase 3C-1 抽取 Due-now / Reset；Phase 3C-2 抽取 Lifecycle Mutation Family | `ReviewCardManage.vue` 1,210 行、11 个 direct `axios.` references、6 个 `v-dialog`；`ReviewCardLifecycleMutationSurface.vue` 414 行、1 个 descriptor GET、2 个 lifecycle POST、3 个对话框；冻结 `expected_version`、防止 descriptor 旧响应覆盖并在成功后清理旧快照 | `docs/plans/review-card-manage-architecture-convergence-plan.md`；ADR-0010、0012–0014、0017；自动回归完成；Chrome DevTools 1 connector 502，authenticated write acceptance pending | Phase 3C-3 Delete Mutation Family Planned / Not Authorized；本轮停止 |
...
| 3 | Browser / ReviewCardManage convergence | Phase 3C-2 Code Complete / Browser Acceptance Pending | Card Info、Search、Table、Due-now / Reset 与 Lifecycle Mutation Surface 已形成独立职责所有者；父组件 1,210 行、11 个请求、6 个弹窗；Phase 3C-3 Delete Mutation Family Planned / Not Authorized；见 `review-card-manage-architecture-convergence-plan.md` |
