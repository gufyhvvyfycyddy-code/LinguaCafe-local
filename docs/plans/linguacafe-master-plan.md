...
> | State | Work |
> |---|---|
> | Production Closed | Queue Order; Custom Study 1A; Manual Sense POS + shared form; Settings architecture convergence; Preset V1A; Preset V1B; Preset V1C; Preset V1D; Browser/ReviewCardManage Phase 3A; Browser/ReviewCardManage Phase 3B-1; Browser/ReviewCardManage Phase 3B-2; Browser/ReviewCardManage Phase 3C-1; Browser/ReviewCardManage Phase 3C-2; Browser/ReviewCardManage Phase 3C-3 |
...
> | Current Phase | No implementation phase is currently authorized; Browser/ReviewCardManage Phase 3C-4 is Accepted / Production Closed; Phase 3D Container Closure is Planned / Not Authorized |
...
| 3 | Browser / ReviewCardManage architecture convergence | Completed / Production Closed through Phase 3C-4 | Phase 3A 抽取只读 Card Info；Phase 3B-1 抽取 Search；Phase 3B-2 抽取 Table；Phase 3C-1 抽取 Due-now / Reset；Phase 3C-2 抽取 Lifecycle Mutation Family；Phase 3C-3 抽取 Delete Mutation Family；Phase 3C-4 抽取 Leech Governance Mutation Family | `ReviewCardManage.vue` 767 行、7 个 direct `axios.` references、2 个 `v-dialog`；`ReviewCardLeechGovernanceMutationSurface.vue` 366 行、2 个请求、2 个弹窗；Leech suspend 继续委托生命周期单一请求所有者；表格继续只拥有当前行、选择和 intent | `docs/plans/review-card-manage-architecture-convergence-plan.md`；`docs/testing/review-card-leech-governance-browser-acceptance-2026-07-18.md`；TDD、自动回归、构建与 authenticated MCP Chrome 验收完成 | Phase 3D Container Closure Planned / Not Authorized；需要单独授权 |
...
| 3 | Browser / ReviewCardManage convergence | Phase 3C-4 Accepted / Production Closed | Card Info、Search、Table、Due-now / Reset、Lifecycle、Delete 与 Leech Governance Mutation Surface 已形成独立职责所有者；父组件 767 行、7 个请求、2 个弹窗；Phase 3D Container Closure Planned / Not Authorized；见 `review-card-manage-architecture-convergence-plan.md` |
