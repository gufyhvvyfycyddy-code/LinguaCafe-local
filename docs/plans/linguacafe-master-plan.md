...
> | State | Work |
> |---|---|
> | Production Closed | Queue Order; Custom Study 1A; Manual Sense POS + shared form; Settings architecture convergence; Preset V1A; Preset V1B; Preset V1C; Preset V1D; Browser/ReviewCardManage Phase 3A; Browser/ReviewCardManage Phase 3B-1; Browser/ReviewCardManage Phase 3B-2; Browser/ReviewCardManage Phase 3C-1; Browser/ReviewCardManage Phase 3C-2 |
...
> | Current Phase | No implementation phase is currently authorized; Browser/ReviewCardManage Phase 3C-2 is Accepted / Production Closed; Phase 3C-3 Delete Mutation Family is Planned / Not Authorized |
...
| 3 | Browser / ReviewCardManage architecture convergence | Completed / Production Closed through Phase 3C-2 | Phase 3A 抽取只读 Card Info；Phase 3B-1 抽取 Search；Phase 3B-2 抽取 Table；Phase 3C-1 抽取 Due-now / Reset；Phase 3C-2 抽取 Lifecycle Mutation Family | `ReviewCardManage.vue` 1,210 行、11 个 direct `axios.` references、6 个 `v-dialog`；`ReviewCardLifecycleMutationSurface.vue` 414 行、1 个 descriptor GET、2 个 lifecycle POST、3 个对话框；它是 ReviewCardManage 域内唯一生命周期请求所有者，独立正式复习入口 `SenseReview.vue` 保留自己的客户端；父组件仍有无可达表格入口的 legacy `/enabled` 兼容代码 | `docs/plans/review-card-manage-architecture-convergence-plan.md`；`docs/testing/review-card-lifecycle-mutation-browser-acceptance-2026-07-17.md`；ADR-0010、0012–0014、0017；自动回归与 authenticated Chrome 双 viewport 验收完成 | Phase 3C-3 Delete Mutation Family Planned / Not Authorized；需要单独授权 |
...
| 3 | Browser / ReviewCardManage convergence | Phase 3C-2 Accepted / Production Closed | Card Info、Search、Table、Due-now / Reset 与 Lifecycle Mutation Surface 已形成独立职责所有者；父组件 1,210 行、11 个请求、6 个弹窗；Phase 3C-3 Delete Mutation Family Planned / Not Authorized；见 `review-card-manage-architecture-convergence-plan.md` |
