...
>
> This file is the long-term product/work ledger, not universal task context. Read it only when the task changes the ledger, compares product directions, selects a phase, or verifies that work was not lost. For bounded implementation or bug fixes, use the owning contract, accepted ADRs, code, tests, and harness. Historical task rows and old imperative wording below do not authorize current work.
>
...
> | State | Work |
> |---|---|
> | Production Closed | Queue Order; Custom Study 1A; Manual Sense POS + shared form; Settings architecture convergence; Preset V1A; Preset V1B; Preset V1C; Preset V1D; Browser/ReviewCardManage Phase 3A; Browser/ReviewCardManage Phase 3B-1; Browser/ReviewCardManage Phase 3B-2; Browser/ReviewCardManage Phase 3C-1; Browser/ReviewCardManage Phase 3C-2; Browser/ReviewCardManage Phase 3C-4; Browser/ReviewCardManage Phase 3D |
...
> | Current Phase | Phase 3–7 authorized scope closed; AI Study Card lifecycle/package/generation/source-binding and disabled-provider preflight Accepted under ADR-0032; real provider deliberately disabled; Browser/ReviewCardManage Phase 3C-3 Delete Mutation Family remains Planned / Not Authorized |
> | Authorized Sequence | Browser/ReviewCardManage Phase 3A–3D → Card Marker + Custom Study 1B → Reviewer convergence → Reader UI/reader architecture → AI Study Card service/provider gate |
> | Environment Gate | Real external AI/provider work is deliberately disabled and requires a future explicit authorization |
>
> Custom Study 1A and 1B are Accepted / Production Closed. 1B adds ReviewCard-owned marker `0..7`, Browser `flag:`, Card Info visibility, the marked preview criterion, and the shared Browser/Sense Review picker without changing FSRS, ReviewLog, lifecycle, or normal queue ownership. Evidence: `docs/testing/review-card-marker-custom-study-browser-acceptance-2026-07-17.md`.
>
> **Architecture status correction**: the old “overall architecture closure 100%” statement is historical. Settings convergence and Browser ownership extraction removed major concentrated debt. The fresh baseline is 28 production files over 500 lines, 10 over 1,000, and 1 over 1,500; current assessment is **6.0/10, localized medium-high burden**. `ReviewCardManage.vue` is 905 lines after Card Marker coordinator wiring, while `TextBlockGroup.vue` (2,514), `SenseReview.vue` (1,476), Reader services and remaining compatibility code still require staged governance.
>
...
| Mgmt-7-b | ✅ 已完成（按学习日精确累计首次新卡引入，正式 sense/legacy 入口共用） |
...
| ReaderSidebar-Boundary-Fix-1 | ✅ 已完成 — 宽屏查词侧栏由 600/560/520px 收窄为 540/500/460px；新增独立的 `panel width` 与 `reservation width` 契约，阅读区为侧栏额外预留 24px 可见边界；`TextBlockGroup.vue` 改为复用共享宽度 helper，移除第三份重复断点。1920×1080 与 1780×861 实测外侧边界约 24.67px，900×900 保持 400px 浮动面板，长句完整换行且无横向溢出。不改查词、WordSense、ReviewCard、ReviewLog、FSRS、API 或数据库。 |
...
| 3 | Browser / ReviewCardManage architecture convergence | Completed / Production Closed for authorized scope | Phase 3A Card Info；Phase 3B-1 Search；Phase 3B-2 Table；Phase 3C-1 Scheduling；Phase 3C-2 Lifecycle；Phase 3C-4 Leech；Phase 3D Container Closure 均已接受 | `ReviewCardManage.vue` 905 行、9 个 direct `axios.` references、4 个 `v-dialog`；Marker 接线后七个职责所有者独立；生命周期写入由 Lifecycle owner 独占 | `docs/plans/review-card-manage-architecture-convergence-plan.md`；3C-2 与 3C-4 真实认证验收；`review-card-leech-mutation-browser-acceptance-2026-07-17.md` | Phase 3C-3 Delete Mutation Family Planned / Not Authorized，未执行 |
| 4 | Card Marker + Custom Study 1B | Accepted / Production Closed | 可标记重点卡，并在 Custom Study 中临时学习已标记卡 | Marker 落在 ReviewCard，和 lifecycle、leech、WordSense status 分离 | ADR-0029；真实 Browser 验收文档 | 完成 |
| 5 | Reviewer architecture convergence | Accepted / Production Closed | 两页共享最小评分请求协调器，Sense Review 复用既有能力对齐 Anki 快捷键 | 不合并产品页；不新增 API/迁移；真实评分+撤销恢复基线 | ADR-0030；`reviewer-convergence-browser-acceptance-2026-07-17.md` | 完成 |
| 6 | Reader UI-7 / Reader UI-8 小步优化 | Accepted / Production Closed | hover 自动查词依赖项收纳；无选词时不显示空侧栏或永久挤压正文 | 必须逐项真实验收，不和阅读主链路大拆分混做 | ADR-0031；Reader acceptance | 完成 |
| 7 | Reader architecture governance | Accepted / Production Closed | Reader 宽度/可见性为纯函数；`TextBlockService` 删除不可达投影、FSRS、短语索引副本 | 45 tests / 169 assertions；生产 build + 浏览器 | ADR-0031；hotspot audit | 完成 |
| 8 | AI Study Card service convergence | Accepted / disabled-provider scope closed | Lifecycle, package/candidate, and generation/source binding have focused owners; the original service is a 195-line compatibility facade; preflight exposes blockers without secrets | None in the authorized disabled-provider scope | ADR-0032 / roadmap Phase 7 / browser acceptance | Complete |
| 9 | Real AI provider / automatic chapter analysis | Deferred by user decision | 显式触发的真实 AI 推荐 | provider、secret、成本、超时、数据边界和 Network 验收 | ADR-0032；V6 plan | Keep disabled until a future explicit authorization |
...
### 7.4 2026-07-10 Anki 对齐审计结论（历史快照，已被当前状态表取代）
...
**当时的 P1 差距（现已关闭）**：报告 deep link、预计间隔、评分撤销、bury / suspend / archive 语义分离和旧 stage 用户入口清理随后均已完成。

**当时的 P2 差距（大部分已关闭）**：Saved Search、遗忘卡治理、Custom Study 1A、未来到期、耗时、可检索率和真实保持率统计随后均已完成。Preset 仍是独立产品 Gate；ReviewCardManage 与 AdminReviewSettings 的进一步拆分只能在实际修改相关领域时按职责进行，不作为无边界重构任务。
...
5. 该复核不授权 deck/subdeck、Reviewer 或 Reader 开发。Preset V1D 已完成；Browser / ReviewCardManage 仅成为下一授权阶段，本轮仍未进入实现。
...
| 3 | Browser / ReviewCardManage convergence | Completed / Production Closed for authorized scope | Card Info、Search、Table、Scheduling、Lifecycle、Leech 与 Marker 已形成独立职责所有者；父组件 905 行、9 个请求、4 个弹窗；Phase 3D 已关闭；Phase 3C-3 Delete Mutation Family Planned / Not Authorized；见 `review-card-manage-architecture-convergence-plan.md` |
...
“总体架构收口 100%”不再作为当前结论。当前可核查事实：28 个生产文件超过 500 行，10 个超过 1000 行，2 个超过 1500 行。代码债务评分为 **6.5/10，局部高负担**。测试、领域模型、事务和权限护栏使项目仍可维护；多个超大 Vue 页面、宽 Service 和过长状态文档使修改成本持续上升。后续架构任务使用文件行数、请求数量、职责数、接口稳定性、测试和 Chrome 验收衡量，不再使用笼统百分比。
...
7. **`tests/js/SenseReviewTodaySummaryGuard.test.mjs`** — 历史实现记录；该 guard 后续已删除，覆盖已迁移到现存的 `SenseReviewDailyReportGuard.test.mjs` 与 `SenseReviewReportCenterGuard.test.mjs`。
...
- 历史 Node guard `SenseReviewTodaySummaryGuard.test.mjs`: 当时 10 passed；该文件后续已删除并迁移覆盖。
- `npm run development`: compiled successfully
...
### Historical roadmap snapshot (superseded; not the current next step)

The numbered list below preserves the earlier planning sequence. Its Custom Study phase labels are historical; the authoritative status at the top of this document controls. No next task is authorized here.
...
5. **Custom Study 1A** — ✅ Accepted / Production Closed。Tasks 2000-15 through 2000-22 established the criteria, candidate queries, immutable token state, preview policy, session ordering, eligibility revalidation, stateless API, and candidate-count contract. Production closure added `/custom-study`, the chapter picker, `CustomStudy.vue`, reusable `SenseStudyCard.vue`, `SenseSentencePreview.vue` reuse, sessionStorage token handling, source-context reuse, and two-viewport MCP Chrome acceptance. The session remains preview-only: it does not write ReviewLog, change FSRS, change lifecycle, or alter the normal queue. Full phase history is archived in `docs/history/custom-study-1a-production-closure-history-2026-07-14.md`; Custom Study 1B and its Card Marker gate remain separate future work.
6. **Saved Search** — ✅ 已完成（ADR-0017）
7. **today-only limits / Mgmt-7-b** — ✅ 已完成（ADR-0018）
8. **Review Time / Study Overview V1** — ✅ 已完成（ADR-0019）

不提前把 Custom Study、Saved Search、today-only limits、Study Overview 或其他后续任务标记完成。Card Info (Task 2000-8) 已完成。Queue Order (Task 2000-9A 架构研究 + Task 2000-9B 架构修正+开发 + Task 2000-10A 生产收口 + Task 2000-13 recovery helper + Task 2000-14 lock fix 最终关闭) ✅ Accepted / 生产验收通过（Task 2000-14，网页端最终验收，2026-07-14）。Custom Study 1A 架构契约已冻结（ADR-0016 + implementation plan）；Phase 1 ✅ Accepted；Phase 2A ✅ Accepted；Phase 2B ✅ Accepted（Task 2000-18：`EloquentChapterLocator` + `SourceChapterQuery` + `LeechAttentionQuery` + `CustomStudyQueryService`，86 tests）；Phase 3A ✅ Accepted / Closed（Task 2000-19 + Task 2000-20 docs closure + Task 2000-22 final closure：`CustomStudySessionState` + `CustomStudySessionTokenService` + `CustomStudySessionStateException` + 2 unit tests；Task 2000-22 补齐 `available_candidate_count` + `withEligibilityResolution()`）；Phase 3B ✅ Accepted / Closed（Task 2000-20：`CustomStudyPreviewPolicy` + `withProgress` + `waitUntil` + `isCompleted` + 51 tests；Task 2000-21 docs/harness 修复；Task 2000-22 final closure + `resolveEligibility()`）；Phase 4A SessionOrder ✅ Accepted / Closed（Task 2000-21：`CustomStudySessionOrder` + 55+ tests；Task 2000-22 final closure）；Phase 4B Backend session vertical slice 代码与测试完成（Task 2000-22：`available_candidate_count` + `withEligibilityResolution()` + `resolveEligibility()` + `CustomStudySessionEligibilityService` + `CustomStudySessionException` + `CustomStudySessionService` open/answer/resume + `CustomStudyController` + 三条 POST 路由 + 6 feature tests + 2 Node guards），等待网页端验收；`candidate_count` 产品 Gate 已关闭 — 冻结 Option A（全部可用候选数，不受 card_limit 截断）；Phase 5-7 未开始，需网页端总流程设计师单独授权才能进入。

Custom Study 1A frontend update (2026-07-14): `/custom-study` now exposes the four frozen criteria, authenticated source-chapter options, a stateless preview session, and source context reuse. Its opaque token lives only in sessionStorage; it calls the existing Custom Study session routes only and leaves formal review, FSRS, ReviewLog, lifecycle, and the normal queue unchanged.

Custom Study 1A final update (2026-07-14): implementation and MCP Chrome
acceptance are complete. The endpoint, aligned example payload, shared card,
and preview UI are all live locally; preview rating left the formal-review
database fingerprint unchanged. Await the web designer's final product
acceptance before starting any follow-up scope.
