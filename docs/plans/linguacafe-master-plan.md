...
| ReviewCardManage-ComplexSmokeAndCompliance-1 | 构造安全测试数据完成 MCP Chrome 页面 smoke（批量归档/恢复/dueNow/reset弹窗）；新增 3 个自动测试（reset message/ bulkEnabled shape/ bulkDestroy skipped）；同步文档并修正 notification script 合规规则（notify.ps1 明确属于 notification script，默认禁止）。 |
