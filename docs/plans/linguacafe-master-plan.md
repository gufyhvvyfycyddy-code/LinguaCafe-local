...
> | Current Phase | Phase 6A Reader hover lookup policy is Accepted / Production Closed; remaining Phase 6 Reader UI and architecture governance is current |
...
| 6 | Reader UI / architecture governance | Current | Phase 6A hover lookup policy 已 Accepted / Production Closed；下一切片继续一次治理一个已表征职责 | 不改变阅读主链路、surface/lemma、WordSense、ReviewCard、ReviewLog 或 FSRS | 同路线 Phase 6 | scoped architecture gate → harness → implementation → browser acceptance |
