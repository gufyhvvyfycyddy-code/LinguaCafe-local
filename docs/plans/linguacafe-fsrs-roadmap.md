...
| TRAE-Smoke-Guard-ChapterId-1 | ✅ 已完成 — 给 text reader smoke guard 增加 `--chapter-id` 参数（默认 5），避免通过修改数据库归属来跑 smoke。测试工具改造，不属于 FSRS 功能本体，不改业务代码，不进入 AI-6 / Lemma-Origin / 架构收敛主任务。起因：Lab-4 因数据库污染被拒绝 |
...
#### Sense 复习系统服务边界文档（✅ 已完成）

- 已新增 [docs/plans/sense-review-service-boundaries.md](./sense-review-service-boundaries.md)
- 用于约束 SenseReviewService、SenseReviewQueryService、ReviewLimitSummaryService、SenseTokenPayloadService、SenseSourceContextService、SenseReviewCardSerializerService 的职责边界
- 这是架构稳定化记录，不属于新的功能开发
- 不进入任何新的 FSRS 功能阶段
...
| 提取服务 | 说明 |
|----------|------|
| ReviewCardManageQueryService | 管理页查询、筛选、排序逻辑从 Controller 抽出 |
| ReviewCardExportService | JSON / Anki TSV / CSV 导出格式逻辑从 Controller 抽出 |
| ReviewCardManageItemSerializerService | 管理页行数据 / 单卡响应 payload 从 Controller 抽出 |

**下一步**：只读侦察危险写操作服务边界（`ReviewCardManage-MutationBoundary-Scout-1`）。

commit: `a3e9836 refactor: extract review card manage item serializer`
