...
**建议下一步**：D.4-d-a — 重排快照表 schema + 创建快照记录（在 confirmAndApply 中写入 snapshot，不实现撤销）。
...
**建议下一步**：D.4-d-a — 重排快照表 schema + 创建快照记录（在 confirmAndApply 中写入 snapshot，不实现撤销）。详见 [fsrs-reschedule-undo-scout.md](./fsrs-reschedule-undo-scout.md)。
