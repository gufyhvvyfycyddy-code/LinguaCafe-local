...
**建议下一步**：D.4-final-review — FSRS D.4 全阶段最终 review。
...
**已实现 D.4-d-a、D.4-d-b、D.4-d-b-fix、D.4-d-c、D.4-d-c-fix、D.4-d-e**：快照表 schema + 撤销后端 API + 事务内校验加固 + 前端撤销入口 + UI修复 + 全链路验收。详见 [fsrs-reschedule-undo-scout.md](./fsrs-reschedule-undo-scout.md)。
**建议下一步**：D.4-final-review — FSRS D.4 全阶段最终 review。
