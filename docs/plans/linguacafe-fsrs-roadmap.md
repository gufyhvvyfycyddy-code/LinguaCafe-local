...
**建议下一步**：D.4-d-b — 撤销后端 API（POST /settings/fsrs/reschedule-undo 路由 + Controller + undo 逻辑）。
...
**已实现 D.4-d-a**：快照表 schema + 创建快照记录。详见 [fsrs-reschedule-undo-scout.md](./fsrs-reschedule-undo-scout.md)。
**建议下一步**：D.4-d-b — 撤销后端 API（POST /settings/fsrs/reschedule-undo 路由 + Controller + undo 逻辑）。
