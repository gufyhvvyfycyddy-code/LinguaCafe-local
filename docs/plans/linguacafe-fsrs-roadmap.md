...
### C.12-e-scout — 复习卡管理页列显示 / 列宽自定义

**状态**：侦察阶段，待冻结

**来源**：C.12-d-a 后管理页列数达到 18 列，用户在浏览器验收时发现"遗忘 / 最近复习 / 到期"需要横向滚动才能看到，操作列 sticky 虽然可用，但关键列不够直观。用户明确提出可隐藏部分长文本列并支持列自定义。
...
1. 列配置应保存在哪里：
   ...
   - 用户设置表
   - 后端 user_settings
   - 先 localStorage 后服务端同步
2. 第一版是否只做前端 localStorage，不改后端。
3. 哪些列允许隐藏，哪些列必须常驻。
4. checkbox 和操作列是否禁止隐藏。
5. ID 是否允许隐藏。
6. 列宽拖拽是否用纯 Vue 事件实现，还是先用预设宽度。
7. 是否需要"恢复默认列设置"按钮。
8. 是否需要"紧凑模式"。
9. 是否需要把低频操作按钮收进"更多"菜单。
10. 表格 min-width 是否应改为动态计算。
11. 列宽调整是否与 sticky 操作列冲突。
12. 是否要把列设置放在"高级筛选"旁边，新增"列设置"按钮。
13. 是否需要在移动/窄屏下强制简化列。
14. 是否影响浏览器验收和截图可读性。

**建议分阶段**：

- C.12-e-a：列显示/隐藏自定义，优先 localStorage，不改后端。
- C.12-e-b：列宽预设或拖拽调整，持久化列宽。
- C.12-e-c：操作列优化，把低频按钮如查看原文、重置、彻底删除收进"更多"菜单。
- C.12-e-d：紧凑模式 / 默认列模板。

**禁止范围**：

1. 不改变 `/review-cards/manage/data` 返回结构。
2. 不改变 ReviewCard / WordSense / ReviewLog 语义。
3. 不改变排序和筛选后端逻辑。
4. 不改变 reset/archive/delete/due-now 语义。
5. 不做导出。
6. 不做详情抽屉。
7. 不做删除复习记录。
8. 不做"新词"等级选项。
