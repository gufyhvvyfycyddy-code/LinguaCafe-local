...
以下任务为候选，均未冻结实现。C.15、C.16、C.17、C.18、C.20、C.20-a、C.21-scout、C.21-a、C.22-scout、C.22-a-lite、C.23-scout、C.24-scout、C.24-a-lite、D.1-scout、D.2-a 已完成。

| 优先级 | 编号 | 内容 | 类型 | 理由 |
|--------|------|------|------|------|
| 高 | D.2-b | Desired Retention 帮助说明增强 | 纯前端 | 帮用户理解 retention 影响 |
...
**建议下一步**：D.2-b — Desired Retention 帮助说明增强（纯前端，最小改动）。
