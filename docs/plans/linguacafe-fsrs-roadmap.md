...
**建议下一步**：D.4-d-c — 前端撤销按钮 + 确认弹窗（AdminReviewSettings.vue）。
...
**已实现 D.4-d-a、D.4-d-b、D.4-d-b-fix**：快照表 schema + 撤销后端 API + 事务内校验加固。详见 [fsrs-reschedule-undo-scout.md](./fsrs-reschedule-undo-scout.md)。
**建议下一步**：D.4-d-c — 前端撤销按钮 + 确认弹窗（AdminReviewSettings.vue）。
