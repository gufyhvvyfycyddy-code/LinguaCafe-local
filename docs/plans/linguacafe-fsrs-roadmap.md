...
**建议下一步**：D.4-c-e — 真实数据联调与浏览器 smoke 总验收。
...
#### D.4-c-d：前端确认按钮 + 二次确认弹窗（🔄 开发中）

- AdminReviewSettings.vue 预览卡片内底部"确认重排"按钮（v-btn color="error" depressed）
- 点击后弹出 v-dialog 二次确认弹窗，复用 DeleteBookDialog 的 3 秒延迟条模式
