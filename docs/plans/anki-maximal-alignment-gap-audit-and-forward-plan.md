...
Current status (2026-07-17): Phase 8A–8C runtime slices are complete. Phase 8D completed read-only reconnaissance and froze the first `source:` contract in `docs/plans/review-card-browser-source-search-semantic-freeze.md`; Phase 8E implemented that exact source contract. Phase 8F froze `missing:definition|example|source` in `docs/plans/review-card-browser-missing-field-search-semantic-freeze.md`; Phase 8G implemented those exact tokens through the existing Parser/Criteria/Query Applier and one shared missing-field predicate owner used by both token and top-filter paths. Phase 8H froze additive Anki-style `rated:<days>` / `rated:<days>:<1..4>` recent formal-review windows in `docs/plans/review-card-browser-recent-review-search-semantic-freeze.md`, while preserving lifetime `rated:again|hard|good|easy`. Phase 8I implemented that exact numeric contract through the existing Parser/Criteria/Query Applier, reused `SenseReviewReportPeriodService` for natural-day boundaries, preserved all four consumers, and completed focused tests, Browser compatibility regression, build, real localhost search/help/Network/Console acceptance, and zero-write checks. The next entry is Phase 8J, read-only reconnaissance and semantic freeze for quoted phrase and negation only; runtime implementation remains unauthorized.
