...
Current status (2026-07-17): Phase 8A–8C runtime slices are complete. Phase 8D completed read-only reconnaissance and froze the first `source:` contract in `docs/plans/review-card-browser-source-search-semantic-freeze.md`; Phase 8E implemented that exact source contract. Phase 8F froze `missing:definition|example|source` in `docs/plans/review-card-browser-missing-field-search-semantic-freeze.md`; Phase 8G implemented those exact tokens through the existing Parser/Criteria/Query Applier and one shared missing-field predicate owner used by both token and top-filter paths. Phase 8H froze additive Anki-style `rated:<days>` / `rated:<days>:<1..4>` recent formal-review windows in `docs/plans/review-card-browser-recent-review-search-semantic-freeze.md`; Phase 8I implemented that exact numeric contract through the shared Browser pipeline and completed runtime/browser acceptance. Phase 8J completed read-only reconnaissance and froze positive quoted phrases plus negated plain text/phrases in `docs/plans/review-card-browser-quoted-phrase-negation-semantic-freeze.md`; Phase 8K implemented that exact linear contract through the shared Parser/Criteria/Query Applier pipeline while preserving existing unquoted text behavior and rejecting advanced-token negation. OR, parentheses, precedence, and a general AST remain excluded. The next entry is Phase 8L, a separately authorized read-only reconnaissance and semantic-freeze step for OR/parentheses; runtime implementation remains unauthorized.
