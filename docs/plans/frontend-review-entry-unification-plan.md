...
> 状态：**第一轮已实现（Round 1 Done）**。旧路由保留，后续 alias / 高级分组细化未实现。V2 AI 示意卡列表/预览入口已在阅读页查词侧栏落地，与复习入口统一协调。
...
- 路线冻结后：前端入口整理 50% → 65%。
- 第一轮最小改法实现并验收后：前端入口整理 92%（V1 任务指定口径）。
- V2 AI 示意卡列表/预览入口落地后：前端入口整理 98%（V2 任务指定口径）。
- 本轮只完成入口统一第一轮 + V2 列表/预览入口协调，不代表最终清理完成。
