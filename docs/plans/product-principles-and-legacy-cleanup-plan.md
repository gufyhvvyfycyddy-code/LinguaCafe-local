...
> **性质**：产品定位 / 长期原则 / 功能行为约束
> **最新更新**：2026-07-03 (GLM-MorphologyLemmaDefectFix-1)
...
7. ~~本轮只写原则和计划，不实现。~~ **2026-07-03 (Trae-LemmaKnownSenseBridge-1) 已实现原则 1-3 + 5-6。Codex-MorphologyMatrix-ImportRegression-1 补齐形态变化测试矩阵与文章 fixture 导入回归：覆盖 ways/technologies、mice/children、studies/watches、ran/went、written/published、running/studying、better/worse、used/broken；测试确认 surface 保留、lemma 显示、添加新释义 payload 优先 lemma 且保留 surface_form。GLM-RealMorphologyImportClickCompletion-1 将形态矩阵升级为真实 tokenizer/importer 覆盖（`ChapterService::processChapterText` 调用真实 Python spaCy tokenizer，覆盖真实导入链路而非纯 fixture 断言），8/8 形态变化类别通过 Playwright 真实页面点击覆盖（18 次真实点击），4 个词性歧义词 `published` / `used` / `broken` / `left` 通过真实页面点击覆盖；data-layer fixture 测试重命名为 `MorphologyMatrixLemmaBridgeDataLayerTest`。真实页面点击验收未使用 API/axios/fetch 模拟点击。原则 4（自动上下文感知绑定）+ AI 建议仍未实现；`published` / `running` / `used` / `broken` 等歧义词不得被测试或产品规则写成不可逆自动绑定。阅读中刷卡评分与 AI 判断熟词僻义仍未实现；不写 ReviewLog、不改 FSRS、不调用 AI。GLM-MorphologyLemmaDefectFix-1 (2026-07-03) 修复 OpenCode-RealClickFinalAudit-3 暴露的 PHP fallback morphology lemma 缺口：根因是 `TextBlockService::conservativeFallbackLemma()` 原本只查不规则表，其他全部返回 surface，导致 Python 服务不可用时 `technologies/watches/stories/bodies/fixes/boxes` 等 lemma 错误。修复用可解释规则（非硬编码本轮测试词）：`lemmaInEcdict()` / `ecdictAvailable()` 改为 protected；新增 ECDICT-gated `-ies → -y/-ie` 规则（仅在 ECDICT 验证候选 lemma 真实存在时才还原），覆盖 `technologies→technology` / `stories→story` / `bodies→body` / `studies→study`；新增 ultra-safe `-ches/-shes/-xes/-zes → strip -es` 规则（确定性后缀剥离），覆盖 `watches→watch` / `fixes→fix` / `boxes→box` / `goes→go`。歧义词场景拆分按 spec 5.3 A/B 处理：形容词化场景（`published author` / `broken window` / `used car` / `left side`）允许保留形容词 lemma；动词/过去分词场景（`was published` / `has broken` / `was used` / `left the room`）应倾向动词 lemma。MCP Chrome 真实页面点击 15 次：13 OK + 2 P2 残留（`has broken → break` 与 `left side → left` 因 EncounteredWord 单 lemma 设计限制），按 P2 残留报告，不强制修改。同时修复测试用户 stale encountered_words 条目（technologies/watches/published 之前 Python 宕机时创建的错误 lemma），是针对测试用户的定向 UPDATE，不是 truncate/drop/delete/migration。新增 `tests/Feature/MorphologyLemmaDefectFixTest.php` 6 个测试方法 / 149 断言，扩充 `tests/Unit/TextBlockFallbackTokenizerTest.php` 4 个新测试方法。旧入口核查：未出现 `旧词条释义/旧版释义/旧版示意/legacy word review`。ReviewLog/ReviewCard/WordSense 点击前后计数一致。不写 ReviewLog、不改 FSRS、不真实调用 AI、不删除 legacy 兼容层、不硬编码本轮测试词作弊。700% 是子阶段提升，不是固定五条主线虚假上涨。阅读中刷卡评分仍未实现；AI 判断熟词僻义仍未实现。**
