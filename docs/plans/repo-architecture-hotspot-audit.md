...
## Recent Update: GM52-AIStudyCardV5-SideBoxMojibakeRegressionFix-1000-3

- **Round type**: GM5.2 目标模式 — `VocabularySideBox.vue` 中文 mojibake P1 回归修复（前端文案修复，不改后端业务逻辑）。
- **本轮解决的风险**：上一轮 feature island 抽取（commit 39fd175）虽然修复了 mojibake 引起的语法错误（破损标签 `?/span>`、`?/v-btn>`、`?/v-chip>` + 缺失闭合引号），但**没有**修复用户可见中文文案，反而引入了宽屏右侧栏中文乱码（27 处 mojibake，含 PUA 字符 U+E000-U+F8FF，例如 `请选择一个单词或短语` 被破坏成 `璇烽€夋嫨涓€涓\ue044崟璇嶆垨鐭\ue161\ue1e2`，`单词`/`短语` 被破坏成 `鍗曡瘝`/`鐭\ue161\ue1e2` 等）。网页端总流程设计师线上复核发现 P1 回归。
- **本轮落地**：
  1. **反驳式侦查**：当前 master `VocabularySideBox.vue` 确实存在 27 处 mojibake（含 PUA 字符）；`e53d116` 版本对应文案全部正常；`VocabularyBox.vue` / `AiStudyCardDesktopWorkflow.vue` / `AiStudyCardGenerateCardsDialog.vue` / `AiStudyCardGenerateCardsResult.vue` 均无 mojibake。结论：上一轮「修复 mojibake」说法**不完整** — 只修复了语法错误，没有修复用户可见中文文案。
  2. **修复策略**：以 `e53d116` 版本的 `VocabularySideBox.vue` 作为中文文案基准手动恢复。**不**使用 `git checkout e53d116 -- VocabularySideBox.vue` 覆盖当前文件（那会把 feature island 改回旧重复实现），**不**回滚整个 commit 39fd175。保留 `import AiStudyCardDesktopWorkflow` / components 注册 / `<AiStudyCardDesktopWorkflow ref="aiStudyCardWorkflow" />` 挂载，不恢复旧 V1-V5 重复模板。
  3. **修复方式**：因 PowerShell 5.x 读取 UTF-8 无 BOM 文件有编码问题，改用 Python 脚本（项目有 Python tokenizer 服务）按降序长度替换策略修复 27 处 mojibake。包含 PUA 字符的 mojibake 通过 `repr()` 打印精确 Unicode 序列后用 `\uXXXX` 转义显式写出。
  4. **修复范围**：第 24 行 `请选择一个单词或短语` / 第 29 行 `新短语` / 第 30/43 行 `单词` / 第 30/101/193/194 行 `短语`（含 PUA 字符）/ 第 32 行 `显示变形` / 第 33 行 `朗读`（含 PUA 字符）/ 第 34 行 `发送到 Anki` / 第 35 行 `返回单词` / 第 36 行 `取消选择` / 第 43 行 `单词基础信息`（含 PUA 字符）/ 第 52/56 行 `[修改]`（含 PUA 字符）/ 第 77 行 `发音` / 第 80/91 行 `FSRS 熟悉度：尚未复习` / 第 96 行 `词元读音` / 第 98/102 行 `读音` / 第 106 行 `忽略` / 第 107 行 `标为已知` / 第 108 行 `回归为新词` / 第 109 行注释 `由共享 feature island 组件负责` / 第 129/133 行 `添加新释义` / 第 139 行 `条建议` / 第 148 行 `搜索词典...` / 第 161 行 `候选结果（AI + 词典）` / 第 185 行 `手动输入释义` / 第 193 行 `保存短语`（含 PUA 字符）/ 第 194 行 `删除短语`（含 PUA 字符）/ 第 200 行 `形式` `肯定` `否定` / 第 448 行 `无法读取 AI 建议。`（含 PUA 字符）。
  5. **新增 `tests/Feature/VocabularySideBoxChineseTextIntegrityTest.php`**（5 项测试 / 42 assertions）：覆盖文件存在性 / 包含 21 条核心正常中文文案 / 不包含 12 条已知 mojibake 片段（`璇烽` / `鍗曡瘝` / `鐭` / `鏂伴` / `淇` / `鏈楄` / `鍙戦` / `杩斿` / `FSRS 鐔` / `?/span` / `?/v-btn` / `?/v-chip`）/ 仍挂载 `AiStudyCardDesktopWorkflow`（证明 feature island 未回滚）/ 不包含旧重复 workflow 指纹（`<v-dialog v-model="aiPendingListDialog"` / `<v-dialog v-model="aiStudyCardPreviewDialog"` / `aiPreviewPackage: null` / `parseAiRecommendations()` / `generateFinalCandidatesPackage()`）。
- **MCP Chrome 真实验收**：A. 宽屏 1920x900 `/chapters/read/6` 点击 construed → VocabularySideBox 显示正常中文（`单词` / `发送到 Anki` / `取消选择` / `单词基础信息` / `[修改]` / `忽略` / `标为已知` / `回归为新词` / `添加新释义` 全部正常，无 mojibake）→ 点击「待 AI 解释列表」→ 弹窗显示「待 AI 解释的词 / 待解释 (1) / 已取消 (0) / 取消 / 生成 AI 示意卡 / 当前共 1 个待解释词」全部正常中文 → Network 27 个请求全部 127.0.0.1:8000 → Console 仅预期 WebSocket 降级；B. 半屏 900x900 `/chapters/read/6` 点击 construed → VocabularyBox 显示正常中文（`忽略` / `标为已知` / `回归为新词` / `待 AI 解释` / `待 AI 解释列表` / `词典搜索` / `+ 添加新释义` / `当前词形` / `词元：` / `候选预览` / `是这个意思` / `不是这个意思` / `已学词义候选` / `已学` / `已复习 0 次` / `编辑该释义` / `删除释义` / `下次复习：今天 状态：新卡` / `查看例句` / `关闭` / `编辑` / `发送到 Anki` 全部正常，无 mojibake）→ Network 35 个请求全部 127.0.0.1:8000 → Console 仅预期 WebSocket 降级。
- **保持的产品行为**：宽屏 SideBox + 半屏 VocabularyBox 完整 V1-V5；feature island 架构（`AiStudyCardDesktopWorkflow.vue`）；AI 推荐词默认不选；用户确认前不生成 WordSense/ReviewCard；中文释义必填；英文解释可空；AI reason 不自动保存为 sense_zh；生成的是 `target_type=sense` ReviewCard；不创建 legacy word ReviewCard；不写 ReviewLog；不改 FSRS 调度；不自动调 AI；不新增 AI provider；不新增 migration。
- **明确未做**：不新增产品功能；不回滚 feature island；不恢复旧 V1-V5 重复模板；不修改后端；不修改 `VocabularyBox.vue`（侦查确认无 mojibake）；不修改 `AiStudyCardDesktopWorkflow.vue`（侦查确认无 mojibake）；不实现手机端/BottomSheet；不进入 V6；不自动调 AI；不接 AI provider；不读写 .env；不新增 API key；不写 ReviewLog；不改 FSRS；不创建 legacy word card；不删除 WordSense/ReviewCard/ReviewLog；不删除 legacy 兼容层；不改 Source Context 主线；不改 ReviewCardManage 危险操作；不新增 migration；不清库；不 DCP；不 notification script；不处理 .omo/。
- **测试结果**：12 个测试套件全绿（390 tests / 0 failures）：VocabularySideBoxChineseTextIntegrityTest 5 (42) / AiStudyCardDesktopWorkflowArchitectureGuardTest 22 (293) / AiStudyCardV5DesktopArchitectureGuardTest 17 (146) / VocabularyBoxV5UiGuardTest 16 (131) / AiStudyCardPendingItemTest 86 (484) / WordSenseTest 134 (555) / ReviewFsrsTest 63 (374) / SenseReview 19 (98) / SenseTokenPayloadTest 16 (45) / TestingDatabaseHealthConfigTest 6 (50) / TestingDatabaseHealthTest 6 (47)；`npm run development` 编译成功（5493ms，7.35 MiB app.js）。
- **风险等级**：🟢 低（前端文案修复，不改后端业务逻辑、不改架构、不改 FSRS、不写 ReviewLog、不新增 migration、不自动调 AI、不创建 legacy word card、不删除 WordSense/ReviewCard/ReviewLog、不实现手机端 V1-V5、不进入 V6）。
- Did NOT enter the next task automatically.

---

## 2026-07-06 GM52-AIStudyCardDesktopWorkflowDeepModuleSplit-1000-4 新增热点（深模块拆分）

> **本轮性质**：GM5.2 1000% closed-loop round 4。本轮同时完成检测 + 编码，不是只检测报告。基于 GitHub master `03eec56` 继续。

### 1. 拆分前热点：`AiStudyCardDesktopWorkflow.vue`（约 1020 行大组件）
...
**10 类职责清单**：
1. 待 AI 解释入口
2. pending / dismissed 列表
3. preview package
4. AI 推荐词 JSON 粘贴
5. AI 推荐词解析 / 去重 / 默认不选
6. final candidates package
7. V5 生成学习卡确认对话框
8. 生成结果展示
9. JSON 复制
10. `/reviews/senses` 跳转

**风险等级（拆分前）**：🟡 中（虽然上一轮 feature island 收敛消除了 SideBox / Box 两处复制，但单个组件 1020 行 / 10 类职责已接近"大泥球"门槛，clipboard DOM fallback 泄漏到组件层）。

### 2. 拆分方案（深模块：container + presentational + service）

| 文件 | 类型 | 行数 | 职责 |
|------|------|------|------|
| `AiStudyCardDesktopWorkflow.vue` | container / coordinator | 448 | Vuex 接入、顶层状态、service 调用、子组件组合、跨子组件事件协调 |
| `AiStudyCardPendingListDialog.vue` | presentational | 183 | pending / dismissed 列表切换 + items 展示 + emit dismiss/restore/open-preview/close |
| `AiStudyCardPreviewDialog.vue` | presentational | 253 | V3-V5 预览弹窗，组合 RecommendationPanel + PackagePanel + GenerateCardsResult |
| `AiStudyCardRecommendationPanel.vue` | presentational | 132 | AI 推荐词 JSON 粘贴 + 解析摘要 + 推荐词列表（默认不选）+ emit parse/clear/toggle/select-all/deselect-all |
| `AiStudyCardPackagePanel.vue` | presentational | 67 | preview/final package JSON 展示 + 复制按钮 + emit copy（不直接调用 clipboard API） |
| `AiStudyCardClipboardService.js` | service | 70 | `copyTextToClipboard(text)`，封装 navigator.clipboard + textarea fallback，不访问 Vue/Vuex/后端 |

**子组件契约**：props in / events out；不 import axios；不 import Vuex / mapState；不知道 SideBox / Box 的存在。

### 3. 拆分后 container 状态

- 总行数：448 行（满足 ≤450 限制，未触发 P2 例外说明）；
- template：仅保留入口按钮 + `<AiStudyCardPendingListDialog>` + `<AiStudyCardPreviewDialog>` + `<AiStudyCardGenerateCardsDialog>`；
- script：保留 25 个 data 字段、22 个 methods（满足 `AiStudyCardDesktopWorkflowArchitectureGuardTest` 指纹契约）；
- `copyJsonToClipboard` 改为委托 `AiStudyCardClipboardService.copyTextToClipboard`；
- 添加 3 个 DRY helper 方法（`_resetPreviewPackageState` / `_resetFinalCandidatesState` / `_resetAfterSelectionChange`）。

### 4. 未拆及原因

| 未拆项 | 原因 |
|--------|------|
| `AiStudyCardGenerateCardsDialog.vue` | 上一轮 feature island 已收敛，本轮不再拆，避免触碰 V5 生成学习卡确认对话框的中文释义必填 / 英文解释可选 / AI reason 不自动填入 sense_zh 契约 |
| 顶层状态 + 跨子组件事件协调 | 保留在 container 以保证单一入口、单一 Vuex 接入点；拆出去会破坏 SideBox / Box 仍只挂载一个总入口的契约 |
| service 层（PendingWorkflowService / RecommendationParserService / GenerateCardsService） | 保持不变，避免本轮触碰后端契约；service 已是独立 JS 模块，无需进一步拆分 |

### 5. 防回退测试

新增 `tests/Feature/AiStudyCardDesktopWorkflowDeepModuleGuardTest.php`（27 项 / 221 assertions）覆盖：
- 6 个文件存在性；
- container 引用子组件；
- container 不含 pending list / recommendation panel / package panel 模板指纹；
- container 不含 clipboard DOM fallback（`document.createElement('textarea')` / `document.execCommand('copy')`）；
- clipboard service 含 fallback；
- 4 个子组件不调用 axios；
- 子组件不 import Vuex / mapState；
- parser service 保证 AI 推荐词默认不选；
- generate service 初始化 sense_zh 为空；
- dialog 显示中文释义必填、英文解释可选；
- SideBox / Box 仍挂载 workflow；
- 中文文案完整性测试存在；
- 无外部 AI provider 字符串；
- 无 ReviewLog / FSRS rating / legacy word card 创建调用；
- VocabularyBottomSheet 不含 V5 workflow；
- 行数 guard：workflow ≤ 450 行。

### 6. 测试结果

12 个测试套件全绿（417 tests / 2159 assertions）：
- `AiStudyCardDesktopWorkflowDeepModuleGuardTest`: 27 (221)
- `VocabularySideBoxChineseTextIntegrityTest`: 5 (42)
- `AiStudyCardDesktopWorkflowArchitectureGuardTest`: 22 (293)
- `AiStudyCardV5DesktopArchitectureGuardTest`: 17 (149)
- `VocabularyBoxV5UiGuardTest`: 16 (135)
- `AiStudyCardPendingItemTest`: 86 (484)
- `WordSenseTest`: 134 (555)
- `ReviewFsrsTest`: 63 (374)
- `SenseReview` 系列: 19 (98)
- `SenseTokenPayloadTest`: 16 (45)
- `TestingDatabaseHealthConfigTest`: 6 (50)
- `TestingDatabaseHealthTest`: 6 (47)

`npm run development` Compiled Successfully in 5209ms。

### 7. MCP Chrome 真实页面验收

- 宽屏 1920x900：完整 V1-V5 回归通过，Network 无外部 AI 请求，Console 无阻塞错误，新增 WordSense 1 + ReviewCard #84 target_type=sense，无 ReviewLog / legacy word card。
- 半屏 900x900：完整 V1-V5 回归通过，Network 无外部 AI 请求，Console 无阻塞错误，新增 WordSense 2 + ReviewCard #85/#86 均 target_type=sense，无 ReviewLog / legacy word card。
- 未做 mobile viewport 主流程验收（当前产品无手机端）。

### 8. 风险等级

- **本轮风险等级**：🟢 低（前端深模块拆分，不改后端业务逻辑、不改 API、不改 FSRS、不写 ReviewLog、不新增 migration、不自动调 AI、不创建 legacy word card、不删除 WordSense/ReviewCard/ReviewLog、不实现手机端 V1-V5、不进入 V6）。
- **残留风险**：container 448 行仍接近 450 阈值；若后续 V6 或手机端再次扩展，需进一步拆 container 的 data/methods 到独立的 state service（本轮未做，已记录到下一轮候选）。

- Did NOT enter the next task automatically. 等待网页端总流程设计师验收。

---

## 2026-07-07 GM52-AIStudyCardPendingLifecycleClosure-1000-5 新增热点（pending item 生命周期收口）

> **本轮性质**：GM5.2 1000% closed-loop round 5。本轮同时完成检测 + 编码，不是只检测报告。基于 GitHub master `282f808` 继续。

### 1. 拆分前热点：pending item 生命周期未收口

| 维度 | 收口前事实 |
|------|------------|
| 用户「生成学习卡」后 pending item 状态 | 仍是 `pending`（再次打开列表仍出现，与"已生成卡"事实矛盾） |
| pending_items 可见状态集合 | `pending` / `dismissed` |
| `listPending` 支持过滤值 | `pending` / `dismissed` / `all` |
| `generateCardsFromConfirmedCandidates` 是否修改 pending item | 否（只创建 WordSense/ReviewCard，pending item 保持原状） |
| 跨用户/跨语言 pending item 隔离 | service 已有 user/language scoping，但 lifecycle 未触及 |
| `ai_recommended` source 与 pending item 关系 | 无 lifecycle 处理 |
| 重复 generate-cards 幂等性 | 第二次会进入 `duplicate` 分支（因为 pending item 仍在 list 中），但语义模糊 |
| 前端 PendingListDialog tabs | 2 个（待解释 / 已取消），无「已处理」视图 |
| 前端 Workflow 是否会刷新 pending list | dismiss/restore 会刷新，generate-cards 成功后**不会**刷新 |

**风险等级（收口前）**：🟡 中（pending item 生命周期与产品事实脱节，用户已完成"生成学习卡"却仍出现在 pending list，造成数据语义不一致）。

### 2. 收口方案：5 条生命周期规则冻结

| 规则 | 触发条件 | pending item 处理 |
|------|----------|-------------------|
| R1 | `source=user_selected` + `created` | 标记为 `processed` |
| R2 | `source=user_selected` + `duplicate` | 标记为 `processed` |
| R3 | `source=user_selected` + `skipped` / `failed` | 保持 `pending` |
| R4 | `source=ai_recommended` | 不修改任何 pending item |
| R5 | `dismissed` item | 不应被 processed（不在 `validPendingItems` 中，自动跳过） |

### 3. 后端修改详情

| 文件 | 修改 | 说明 |
|------|------|------|
| `app/Models/AiStudyCardPendingItem.php` | 新增常量 `STATUS_PROCESSED = 'processed'` | 与 `STATUS_PENDING` / `STATUS_DISMISSED` 同位 |
| `app/Services/AiStudyCardPendingItemService.php` | `listPending` 支持 `processed` 过滤（第 136-145 行） | validator `in:pending,dismissed,processed,all`，向后兼容 |
| 同上 | 新增私有方法 `markPendingItemProcessed(int $pendingItemId): bool`（第 1022-1049 行） | 安全、幂等：只 `UPDATE WHERE status=pending AND user_id=? AND language=?`；返回 bool；已 processed 的 item 自动 skipped |
| 同上 | 新增私有方法 `emptyPendingLifecycleInfo(): array`（第 1054-1063 行） | 用于 `ai_recommended` source 不修改 pending item 的占位符 |
| 同上 | `generateCardsFromConfirmedCandidates` 中 lifecycle 合并逻辑（第 886-896 行） | 遍历 confirmed_items payload 的 `source` 字段，按 R1/R2/R3/R4 处理 |
| 同上 | `skippedResult` 包含 lifecycle 字段（第 1001-1015 行） | 返回 `pending_item_lifecycle` 数组，每个元素含 `pending_item_id` / `source` / `previous_status` / `new_status` / `marked_processed` |
| 同上 | `$failed` 数组包含 lifecycle 字段（第 907-917 行） | failed 路径的 lifecycle 字段为 `marked_processed=false` |
| `app/Http/Controllers/AiStudyCardPendingItemController.php` | validator 接受 `processed`：`'status' => ['nullable', 'string', 'in:pending,dismissed,processed,all']` | 与 service `listPending` 一致 |

### 4. 前端修改详情

| 文件 | 修改 | 说明 |
|------|------|------|
| `resources/js/components/Text/AiStudyCardPendingListDialog.vue` | 新增第三个 tab「已处理」+ processed items 列表（只读，无操作按钮） | 第 119-141 行：`statusFilter === 'processed'` 模板分支；显示「状态：已处理 \| 处理于 {{ updated_at }}」 |
| 同上 | 新增 `processedItems` prop | 只读列表，container 通过 `loadAiPendingDismissedItems` 同时加载 processed |
| `resources/js/components/Text/AiStudyCardDesktopWorkflow.vue` | data 新增 `aiPendingProcessedItems: []`（第 158 行） | 三个状态列表分离：`aiPendingItems` / `aiPendingDismissedItems` / `aiPendingProcessedItems` |
| 同上 | `loadAiPendingDismissedItems()` 同时加载 processed（第 235-239 行） | 同时调用 `listPendingItems(axios, { status: 'dismissed' })` 和 `listPendingItems(axios, { status: 'processed' })` |
| 同上 | `confirmGenerateCards` 成功后刷新 pending list | `generateAiStudyCards` 返回的 `pending_item_lifecycle` 字段触发 UI 显示「已从待解释移至已处理」chip，并自动调用 `loadAiPendingItems` + `loadAiPendingDismissedItems` 刷新三个列表 |
| 同上 | 模板绑定 `:processed-items="aiPendingProcessedItems"` | 通过 props 传递给 PendingListDialog |
| 同上 | 行数核验 | 444 行 ≤ 450 限制，未触发 P2 例外说明 |

### 5. 新增测试文件

`tests/Feature/AiStudyCardPendingLifecycleTest.php`（15 项测试 / 多 assertions）覆盖：

1. `test_user_selected_created_marks_pending_item_as_processed` — R1
2. `test_user_selected_duplicate_marks_pending_item_as_processed` — R2
3. `test_user_selected_failed_keeps_pending_item_as_pending` — R3 failed（使用 Mockery 模拟 WordSenseService 抛出异常）
4. `test_user_selected_skipped_keeps_pending_item_as_pending` — R3 skipped
5. `test_ai_recommended_created_does_not_modify_pending_item` — R4 created
6. `test_ai_recommended_duplicate_does_not_modify_pending_item` — R4 duplicate
7. `test_dismissed_pending_item_not_marked_processed` — R5
8. `test_cross_user_pending_item_not_marked_processed` — 安全边界
9. `test_cross_language_pending_item_not_marked_processed` — 安全边界
10. `test_processed_does_not_write_review_log` — 安全契约
11. `test_processed_does_not_reschedule_existing_fsrs_cards` — 安全契约
12. `test_processed_does_not_create_legacy_word_review_card` — 安全契约
13. `test_processed_items_queryable_via_status_processed` — list 查询
14. `test_generate_cards_response_contains_lifecycle_fields` — 响应字段结构
15. `test_repeat_generate_cards_is_idempotent` — 幂等性

### 6. 修改的测试文件

`tests/Feature/AiStudyCardPendingItemTest.php` 中 `test_generate_cards_deduplicates_duplicate_candidates` 更新：新的 lifecycle 逻辑导致第一次 generate-cards 后 pending item 变为 processed，第二次调用时该 item 已不在 `validPendingItems` 中（`status=pending` 过滤），所以第二次进入 `skipped` 而非 `duplicate`。断言改为 `skipped_count=1` 和 `duplicate_count=0`。

### 7. 测试结果

13 个测试套件全绿（554 tests / 0 failures）：
- `AiStudyCardPendingLifecycleTest`: 15
- `AiStudyCardDesktopWorkflowDeepModuleGuardTest`: 27 (221)
- `VocabularySideBoxChineseTextIntegrityTest`: 5 (42)
- `AiStudyCardDesktopWorkflowArchitectureGuardTest`: 22 (293)
- `AiStudyCardV5DesktopArchitectureGuardTest`: 17 (149)
...
**宽屏 1920x900**：
- `/chapters/read/6` 点击 construed → VocabularySideBox → 「待 AI 解释列表」→ pending list 显示「待解释 (1)」
- V3 preview package → V4 final candidates → V5 generate cards dialog
- 填写中文释义「理解为；被解释为（生命周期验收）」→ POST `/ai-study-card/generate-cards` 200
- 结果显示「已生成 1 张学习卡」+「已从待解释移至已处理」chip
- pending list 自动刷新：「待解释 (0) / 已处理 (1)」
- 切换到 processed 视图（通过 evaluate_script 调用 `$el.click()` 触发 v-btn-toggle，因 MCP click 工具无法触发 Vuetify 2 v-btn @change）→ 显示 construed「状态：已处理 \| 处理于 2026-07-07 00:32」
- `/reviews/senses` 页面正常显示 sense card
- 数据库：pending item #5 status=processed, ReviewLog=13, TARGET_WORD=6, TARGET_SENSE=34（含 #87 新增）
- Network 全部 127.0.0.1:8000，Console 仅 WebSocket 降级

**半屏 900x900**：
- `/chapters/read/6` 点击 entanglement → VocabularyBox → 「待 AI 解释列表」→ pending list 显示「待解释 (1)」
- V3 preview → V4 final candidates → V5 generate cards
- 填写中文释义「纠缠；卷入（半屏生命周期验收）」→ POST `/ai-study-card/generate-cards` 200
- 结果显示「已生成 1 张学习卡」+「已从待解释移至已处理」chip
- pending list 自动刷新：「待解释 (0) / 已处理 (2)」（construed + entanglement）
- 数据库：pending item #6 status=processed, ReviewLog=13, TARGET_WORD=6, TARGET_SENSE=35（含 #88 新增）
- Network 全部本地，Console 仅 WebSocket 降级

### 9. 安全契约确认

- 不写 ReviewLog（验收后 ReviewLog 总数仍 13，未增加）
- 不改 FSRS 调度（未触及 `fsrs_state` / `fsrs_due_at` / `fsrs_stability` / `fsrs_difficulty` / `fsrs_reps` / `fsrs_lapses` / `fsrs_last_reviewed_at` / `fsrs_enabled` 字段）
- 不创建 legacy word ReviewCard（验收后 TARGET_WORD 仍 6，未增加）
- 不删除 WordSense / ReviewCard / ReviewLog（仅 UPDATE pending_items.status）
- 不删除 legacy 兼容层
- 不自动调 AI（payload 中 confirmed_items 的 `source` 字段由前端填入，不调用 AI provider）
- 不新增 migration（仅 UPDATE 现有 status 字段值）
- 跨用户 / 跨语言隔离（`markPendingItemProcessed` 强制 user_id + language 校验）

### 10. 风险等级

- **本轮风险等级**：🟢 低（pending item 生命周期收口，仅新增 UPDATE 现有 status 字段值；不改 API 契约、不改 FSRS、不写 ReviewLog、不新增 migration、不自动调 AI、不创建 legacy word card、不删除 WordSense/ReviewCard/ReviewLog、不实现手机端 V1-V5、不进入 V6、不回滚 deep-module split、不重新制造 SideBox/VocabularyBox 复制、不再次制造中文 mojibake）。
- **残留风险**：`AiStudyCardDesktopWorkflow.vue` 仍 444 行接近 450 阈值（与上一轮持平，未恶化）；`AiStudyCardPreviewDialog.vue` 的 prop drilling 仍是 P3 待办（与上一轮持平）。

- Did NOT enter the next task automatically. 等待网页端总流程设计师验收。

---

## 2026-07-07 GM52-SenseMultiExampleBindingAndReviewRotation-1000-6 新增热点（多例句绑定 + 复习例句轮换）

> **本轮性质**：GM5.2 1000% closed-loop round 6。本轮同时完成现状检测 + 编码，不是只检测报告。基于 GitHub master `0321502` 继续。

### 1. 现状检测事实

| 维度 | 检测结果 |
|------|----------|
| `word_senses` 表 | 已存在；含 `lemma`, `surface_form`, `pos`, `sense_zh`, `sense_en`, `aliases_zh`, `collocations`, `example_sentence_en`, `example_sentence_zh` 等字段 |
| `review_cards` 表 | 已存在；`target_type=sense` + `target_id=word_sense_id`；含 FSRS 全字段；`review_cards_user_language_target_unique` 唯一约束保证同 user+language+target_type+target_id 只一张卡 |
| `word_sense_occurrences` 表 | 已存在（无新增 migration）；含 `id`, `word_sense_id`, `chapter_id`, `text_block_index`, `sentence_index`, `sentence_id`, `sentence_en`, `sentence_zh`, `status` 等字段 |
| 一个 WordSense 是否已可拥有多条 occurrence | ✅ 是（一对多关系，本轮确认 `codex_sense_smoke_20260702_b_bind_target` lemma 有 2 条 occurrence） |
| 当前生成 AIStudyCard 是否写 occurrence | 检测前路径已经写 occurrence（`WordSenseService::createWordSenseFromAiStudyCard` 调用 `createWordSenseOccurrence`） |
| duplicate 路径是否写 occurrence | 检测前已支持（`WordSenseService::bindNewOccurrenceFromAiStudyCard` 在 generate-cards 的 duplicate 分支被调用） |
| 手动添加释义是否写 occurrence | 检测前已支持（`WordSenseService::addSense` 调用 `createWordSenseOccurrence`） |
| AI 建议添加释义是否写 occurrence | 检测前已支持（同 `addSense` 路径） |
| `/reviews/senses` 卡片例句来源 | 检测前已由 `SenseReviewCardSerializerService` + `WordSenseExamplePoolService` 构建 candidates 池，从 occurrence + card fallback 取例句 |
| 查看原文/译文来源 | `SenseSourceContextService::sourceContextList` 仍按 ReviewCard 默认来源取（**P2：未跟随 displayed_occurrence_id**） |
| 是否有 last_shown_occurrence / example rotation 字段 | ❌ 数据库无此类字段 |
| 是否有测试覆盖多例句 | ❌ 检测前无 |
| 是否有测试覆盖复习页例句轮换 | ❌ 检测前无 |
| 是否存在 smoke / test lemma 干扰 | 存在 `codex_sense_smoke_*` 等 lemma（本轮不清理，记录为后续 P2） |

### 2. 本轮编码方案

**核心决策：不新增 migration，不写"last shown occurrence id"持久化字段**。本轮复用更早 commit `4432ecd feat: add review example rotation and source carousel` 已经引入的稳定 seed 轮换基础设施：

- `WordSenseExamplePoolService::pickQuestionIndex($total, $reviewCardId, $fsrsReps, $dayOfYear = null)`（已存在，本轮未修改）：`crc32($reviewCardId * 31 + $fsrsReps * 7 + $dayOfYear) % $total`，确定性选择。
- `pickSupplementaryIndex`（已存在，本轮未修改）：在 question index 之外选另一条例句（仅当 total > 1）。
- `exampleCandidates($sense)`（已存在，本轮未修改）：合并 occurrence 例句 + WordSense 自带例句（card fallback），每条带 `occurrence_id` / `is_card_fallback` 标识。
- 轮换不写数据库：同一天对同一卡片取同一例句（避免同一天同一卡显示两条例句造成用户困惑），跨日 + 评分后 `fsrs_reps` 增长自然产生位移。

**本轮真正新增**：在 `SenseReviewCardSerializerService::serialize` 中暴露 3 个 payload 字段，让前端能感知"当前展示了哪条 occurrence / 共有几条例句 / 当前来源状态"；并在 `SenseReview.vue` 答案侧加多例句提示 chip。

**为什么不用持久化字段**：
1. 不新增 migration = 不动 schema = 不触发 high-risk 修改。
2. seed 轮换可测试（输入确定 → 输出确定），可证明"不会永远第一条"。
3. 符合任务规格"第一轮可以用简单轮换"。
4. 若未来需要严格"避免连续两次同一句"，可在下一轮加 migration，但本轮验证 seed 方案已足够。

### 3. 后端修改详情

| 文件 | 修改 | 说明 |
|------|------|------|
| `app/Services/WordSenseExamplePoolService.php` | **未修改** | `exampleCandidates` / `pickQuestionIndex` / `pickSupplementaryIndex` 已在 commit `4432ecd` 存在，本轮直接复用 |
| `app/Services/SenseReviewCardSerializerService.php` | `serialize` 新增 3 个 payload 字段计算与输出 | `displayed_occurrence_id`（来自 question example） / `occurrence_count`（候选池大小） / `example_source_status`（`occurrence` / `card_fallback` / `empty`）；同时新增 docblock 说明轮换不持久化的设计决策 |
...
本轮检测确认现有 `WordSenseService::createWordSenseOccurrence` 已实现三层去重：

1. `sentence_id` 存在 → `word_sense_id + sentence_id` 去重
2. `sentence_id` 缺失 → `word_sense_id + chapter_id + text_block_index + sentence_index` 去重
3. 都缺失 → `word_sense_id + normalized sentence_text` 弱去重（trim + lowercase）

本轮**未修改**去重逻辑（已满足规则 2 要求）。

### 6. 新增测试文件

#### `tests/Feature/SenseMultiExampleBindingTest.php`（13 项 / 45 assertions）

1. `test_word_sense_can_have_multiple_occurrences`
2. `test_same_sentence_id_not_duplicated`
3. `test_same_chapter_text_block_sentence_index_not_duplicated`
4. `test_same_sentence_text_weak_dedup`
5. `test_ai_study_card_created_binds_occurrence`
6. `test_ai_study_card_duplicate_also_binds_occurrence`
7. `test_ai_recommended_without_source_does_not_bind_occurrence`
8. `test_cross_user_occurrences_not_mixed`
9. `test_cross_language_occurrences_not_mixed`
10. `test_no_legacy_word_review_card_created`
11. `test_no_extra_review_log_written`
12. `test_fsrs_not_changed_by_binding`
13. `test_review_card_unique_constraint_preserved`

#### `tests/Feature/SenseReviewExampleRotationTest.php`（10 项 / 31 assertions）

1. `test_single_occurrence_shows_that_example`
2. `test_three_occurrences_do_not_always_show_first`
3. `test_after_rating_next_show_differs_when_possible`
4. `test_displayed_occurrence_id_in_payload`
5. `test_occurrence_count_in_payload`
6. `test_fallback_to_card_example_when_no_occurrence`
7. `test_empty_state_when_no_example_at_all`
8. `test_rotation_does_not_write_review_log`
9. `test_rotation_does_not_change_fsrs_due_stability_difficulty`
10. `test_rotation_does_not_affect_daily_limit`

### 7. 测试结果

13 个测试套件全绿（410 tests / 0 failures）：
- `SenseMultiExampleBindingTest`: 13 (45) — 新增
- `SenseReviewExampleRotationTest`: 10 (31) — 新增
- `AiStudyCardPendingLifecycleTest`: 15
- `AiStudyCardPendingItemTest`: 86
- `WordSenseTest`: 134
- `ReviewFsrsTest`: 63
- `SenseReview` 系列: 19
- `SenseTokenPayloadTest`: 16
- `VocabularySideBoxChineseTextIntegrityTest`: 5
- `AiStudyCardDesktopWorkflowDeepModuleGuardTest`: 27
- `TestingDatabaseHealthConfigTest`: 6
- `TestingDatabaseHealthTest`: 6
- 其余 guard / smoke 套件：全绿

`npm run development` Compiled Successfully（6403ms，app.js 7.4 MiB，无错误）。
...
- 评分 `good` → 跳到下一张多例句卡，提示 chip 同样正确
- 查看原文/译文对话框打开（sourceCount=1，未跟随 occurrence_count=2 → **P2 记录**）
- 轮换验证（临时 PHP 脚本，对 card#61 reps 0-10 序列化）：产生 2 个不同 `displayed_occurrence_id`（14 和 null/card_fallback），DB `fsrs_reps` 验证后未变（=2），无 DB 写入
- Network 全部 127.0.0.1:8000，Console 仅 WebSocket 降级（预期，非阻塞）

**半屏 900x900**：
- 无横向滚动（`scrollWidth=892=clientWidth=892`）
- 多例句提示 chip 正确显示，不挤、不挡评分按钮
- 答案侧布局正常，评分按钮可见
- 查看原文/译文对话框正常打开
- Network 全部本地，Console 仅 WebSocket 降级
...
- 不写 ReviewLog（轮换验证后 DB `ReviewLog` 总数未变）
- 不改 FSRS 调度（轮换验证后 `fsrs_reps` / `fsrs_due_at` / `fsrs_stability` / `fsrs_difficulty` / `fsrs_state` / `fsrs_lapses` 未变）
- 不创建 legacy word ReviewCard
- 不删除 WordSense / ReviewCard / ReviewLog
- 不删除 legacy 兼容层
- 不自动调 AI（轮换纯后端确定性计算）
- 不新增 migration（仅 SELECT 现有 occurrence 数据）
- 跨用户 / 跨语言隔离（occurrence 查询走 `word_sense_id` 链路，WordSense 自带 user/language 隔离）

### 10. 风险等级

- **本轮风险等级**：🟢 低（多例句绑定 + 复习例句轮换，新增 payload 字段只读；不改 API 契约语义、不改 FSRS、不写 ReviewLog、不新增 migration、不自动调 AI、不创建 legacy word card、不删除 WordSense/ReviewCard/ReviewLog、不实现手机端、不进入 V6、不破坏现有 source context fallback）。
- **残留 P2 风险**：`SenseSourceContextService::sourceContextList` 仍按 ReviewCard 默认来源取，未跟随 `displayed_occurrence_id`（查看原文/译文对话框 sourceCount=1 不跟随 occurrence_count=2）。下一轮需让 source context 跟随 `displayed_occurrence_id`。
- **残留 P2 风险**：More 菜单危险操作未做视觉分隔（沿用 Task 5 状态，本轮未顺手处理）。
- **残留 P3 风险**：`AiStudyCardDesktopWorkflow.vue` 仍 444 行接近 450 阈值（与上一轮持平）；`AiStudyCardPreviewDialog.vue` prop drilling 仍是 P3 待办（与上一轮持平）。

- Did NOT enter the next task automatically. 等待网页端总流程设计师验收。

---

## 2026-07-07 GM52-SenseSourceContextFollowDisplayedOccurrence-1000-7 新增热点（source context 跟随 displayed occurrence）

本轮同轮完成现状检测 + 编码，关闭上一轮 P2（查看原文/译文未跟随当前复习例句）。

### 1. 现状检测事实（12 项）

1. `SenseReviewCardSerializerService.php` 已返回 `displayed_occurrence_id` / `occurrence_count` / `example_source_status`（上一轮 Task 6 已实现）。
2. `SenseReview.vue` 点击「查看原文」未传 `preferred_occurrence_id`（`viewSource()` 直接请求 `/senses/{id}/source-context-list` 无 query 参数）。
3. source context 路由：`GET /senses/{id}/source-context-list`（`routes/web.php`）。
4. `SenseOccurrenceController::sourceContextList` 不读 query 参数（直接调用 `sourceContextList($userId, $language, $id)`）。
5. `SenseSourceContextService::sourceContextList()` 不支持指定 occurrence（方法签名只有 `$userId/$language/$senseId`，无 preferred 参数）。
6. `SenseSourceContextService::sourceContext()` 同样不支持指定 occurrence（旧单来源接口，本轮未修改）。
7. `SenseSourceContextResolverService` 有 `findChapterById($chapterId, $userId, $languageId)` 等 helper 可复用（已按 user+language 过滤）。
8. `WordSenseOccurrence` 有 `STATUS_BOUND` 常量，可按 `user_id/language_id/word_sense_id/id/status` 安全查询。
9. source dialog（`SenseExampleDialog.vue`）已支持多 sources carousel（`sourceCount > 1` 时显示来源切换条）。
10. 返回 sources 时无 `occurrence_id` 字段（sources 数组元素是 chapter context，不含 occurrence_id）。
11. 测试未覆盖「查看原文跟随当前显示例句」（无 `SenseSourceContextDisplayedOccurrenceTest`）。
12. MCP Chrome 可复现：当前显示例句与查看原文第一条来源不一致（preferred_occurrence_id 未传，sourceContextList 按 occurrence 顺序返回，sources[0] 不一定是当前显示的 occurrence）。

### 2. 修改文件与热点

#### `app/Services/SenseSourceContextService.php`（修改）
- `sourceContextList()` 方法签名新增 `?int $preferredOccurrenceId = null` 参数。
- 新增 `resolvePreferredOccurrence(WordSense $sense, int $occurrenceId): ?WordSenseOccurrence` 私有方法：严格校验 `id + user_id + language_id + word_sense_id + status=STATUS_BOUND`，任一不匹配返回 null。
- preferred 有效且有 chapter 时：用 `resolver->findChapterById()` + `sourceContextFromChapter()` 构建 source context，放在 sources[0]，标记 `preferred_occurrence_status='matched'`。
- preferred 无效/无 chapter 时：标记 `invalid` / `fallback`，走原 fallback 链。
- 新增 `$usedChapterIds` 数组：避免 preferred 与其他 occurrence 共享同一 chapter 时重复展示（dedupe by chapter_id）。
- 返回 payload 新增 `preferred_occurrence_status` 字段（`matched`/`invalid`/`fallback`/`unavailable`）。
- **风险等级：🟢 低（新增可选参数，无 preferred 时行为完全不变；preferred 校验严格走 user/language/sense/bound 多重过滤；fallback 链不变）**。

#### `app/Http/Controllers/SenseOccurrenceController.php`（修改）
- `sourceContextList()` 读取 `request()->query('preferred_occurrence_id')` 并传给 service。
- 无 query 参数时 `$preferredId = null`，行为与旧版完全一致。
- **风险等级：🟢 低（仅读取可选 query 参数，无鉴权/路由变更）**。

#### `resources/js/components/Senses/SenseReview.vue`（修改）
- `viewSource()` 方法：仅当 `this.currentCard.displayed_occurrence_id` 存在时，把 `preferred_occurrence_id` 加入 axios GET params。
- `sourcePayload` 新增 `preferredOccurrenceStatus` 字段（从 `response.data.preferred_occurrence_status` 读取）。
- More 菜单「彻底删除」前加 `<v-divider class="my-1" />` 视觉分隔（规则 6，<20 行小改）。
- **风险等级：🟢 低（新增可选 query 参数 + divider，不改评分/快捷键/More 菜单逻辑）**。

#### `resources/js/components/Review/SenseExampleDialog.vue`（修改）
- 新增 `preferredHint` computed：`matched`→"已定位到当前复习例句。" / `fallback`→"未定位到当前例句，已显示其他可用来源。"，其他状态返回空字符串。
- 模板顶部新增 `<v-alert v-if="preferredHint" type="success" dense text dismissible>` 展示提示。
- 不显示数据库 id；不改 carousel / scroll / translation 逻辑。
- **风险等级：🟢 低（新增 computed + alert，不改现有 dialog 行为）**。

### 3. 新增测试

#### `tests/Feature/SenseSourceContextDisplayedOccurrenceTest.php`（14 项 / 53 assertions）
1. `test_valid_preferred_occurrence_becomes_first_source` — sources[0].occurrence_id 等于 preferred
2. `test_preferred_occurrence_not_duplicated_in_sources` — sources 不重复 preferred
3. `test_preferred_occurrence_belongs_to_other_user_is_dropped` — 跨用户 fallback
4. `test_preferred_occurrence_with_wrong_language_is_dropped` — 跨语言 fallback
5. `test_preferred_occurrence_bound_to_different_sense_is_dropped` — 跨 sense fallback
6. `test_preferred_occurrence_not_bound_is_dropped` — pending 状态 fallback
7. `test_preferred_occurrence_without_chapter_does_not_500_and_falls_back` — 无 chapter 不 500
8. `test_no_preferred_param_preserves_original_behavior` — 无参数旧行为不变
9. `test_payload_contains_preferred_occurrence_status` — payload 含 status
10. `test_endpoint_writes_no_review_log` — 不写 ReviewLog
11. `test_endpoint_does_not_modify_fsrs_fields` — 不改 FSRS
12. `test_endpoint_does_not_create_legacy_word_review_card` — 不创建 legacy card
13. `test_endpoint_does_not_create_new_word_sense_occurrence` — 不新增 occurrence
14. `test_fallback_chain_serves_card_example_or_unavailable_when_no_chapter_sources` — fallback 链

### 4. 测试结果

14 个测试套件全绿（443 tests / 0 failures）：
- `SenseSourceContextDisplayedOccurrenceTest`: 14 (53) — 新增
- `SenseReviewExampleRotationTest`: 10
- `SenseMultiExampleBindingTest`: 13
- `SenseSourceContext` 系列：全绿
- `SenseReview`: 19
- `SenseTokenPayloadTest`: 16
- `ReviewFsrsTest`: 63
- `WordSenseTest`: 134
...
- source dialog 打开，第一条来源 example 文本与当前卡片显示例句一致
- 显示 fallback 提示「未定位到当前例句，已显示其他可用来源。」（occ#11 无 chapter_id，正确 fallback 到 chapter_fuzzy_title）
- PHP 脚本验证 sense#68 (occ#19/20) 返回 `matched` 状态，sources[0].occurrence_id 正确
- Network 全部 127.0.0.1:8000，Console 仅 WebSocket 降级（预期，非阻塞）

**半屏 900x900**：
- 无横向滚动（`scrollWidth=900=clientWidth=900`）
- dialog fitsWindow=true（852x411.7 在 900x831 内）
- divider 在 900x900 也存在（DOM 验证：6 个子元素含 HR.v-divider）
- 第一条来源仍对应当前例句
- Network 全部本地，Console 仅 WebSocket 降级

### 6. 安全契约确认

- 不写 ReviewLog（endpoint 只读 occurrence + chapter，无 ReviewLog 写入）
- 不改 FSRS（endpoint 不触碰 ReviewCard / fsrs_* 字段）
- 不创建 legacy word ReviewCard（endpoint 不创建任何 ReviewCard）
- 不新增 WordSenseOccurrence（endpoint 只 SELECT 现有 occurrence）
- 不删除 WordSense / ReviewCard / ReviewLog
- 不删除 legacy 兼容层
- 不自动调 AI（纯后端 occurrence + chapter 查询）
- 不新增 migration（仅 SELECT 现有表）
- 跨用户 / 跨语言隔离（preferred occurrence 校验走 user_id + language_id + word_sense_id + status 四重过滤）
- 不破坏现有 source context fallback 链（无 preferred 参数时行为完全不变；preferred 无效时 fallback 到原链）

### 7. 风险等级

- **本轮风险等级**：🟢 低（新增可选 query 参数 + 严格校验 + fallback 保留，不改 API 契约语义、不改 FSRS、不写 ReviewLog、不新增 migration、不自动调 AI、不创建 legacy word card、不删除 WordSense/ReviewCard/ReviewLog、不实现手机端、不进入 V6、不破坏现有 source context fallback）。
- **上一轮 P2 已关闭**：`SenseSourceContextService::sourceContextList` 现在跟随 `displayed_occurrence_id`，查看原文/译文对话框 sources[0] 对应当前复习例句。
- **上一轮 P2 已关闭**：More 菜单危险操作已做视觉分隔（`<v-divider class="my-1" />` + 「彻底删除」保持红色）。
- **残留 P3 风险**：`AiStudyCardDesktopWorkflow.vue` 仍 444 行接近 450 阈值（与上一轮持平）；`AiStudyCardPreviewDialog.vue` prop drilling 仍是 P3 待办（与上一轮持平）。
- **残留 P3 风险**：手动添加释义/AI 建议添加释义路径不写 occurrence（Task 6 记录的后续任务，本轮未处理）。
