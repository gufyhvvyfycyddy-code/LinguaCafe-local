...
**新的最推荐下一阶段**：候选 A（TextBlockService-TokenizerFallbackScouting-1 — 补 fallback 测试缺口）或候选 B（ReviewCardManage-LogsContractTests-1 — 补 logs payload contract tests）或候选 C（SenseReview-FullMenuSmoke-1 — 完整 More 菜单和 occurrence 写入路径 smoke）。
...
- The previously open SenseReview full menu / occurrence write smoke is now covered by a local marker-data command, a command feature test, and a real-page smoke playbook.
- Real browser acceptance covered due-card rating, More/source fallback, confirm, ignore, reject, rebind, and create-new occurrence paths.
- The implementation intentionally stayed out of Vue components, FSRS scheduling semantics, WordSense delete/archive/restore semantics, schema, and AI study card behavior.
