...
下一轮架构优化（含 GLM 1000% 子阶段）必须遵守总设计师设定的高内聚低耦合规则（详见 `vibe-coding-collaboration-rules.md` §27），以下边界适用于本仓库所有架构优化任务：

1. **不允许碰的模块**：FSRS 调度算法、ReviewLog 写入语义、WordSenseService::removeSenseFromReviewSystem 核心删除逻辑、ReviewCardService::resetCardToNew 核心重置语义。
2. **拆分前必须已经有 characterization tests 的模块**：TextBlockService、VocabularyService、DictionaryImportService 拆分前必须先补 contract tests 锁定当前行为。
3. **拆分后必须 observable behavior 不变的模块**：所有 Service 拆分后必须保持 public 方法签名和返回值不变。
4. **只读 Service 可以随时抽**：`WordSenseKnownSenseService` 模式（只读 + `read_only=true` + 不写数据 + 不调 AI）可以安全复制到其它只读查询上。
5. **MCP 形态测试必须使用新词**：每轮使用不同测试词，不重复上一轮全部词汇。
6. **拆分 Vue 组件前必须先 WorkBuddy 确认 UX**：`VocabularySideBox.vue` / `VocabularyBox.vue` 等大组件拆分前必须由 WorkBuddy 网页端体验师验收当前用户操作路径，确保拆分不影响核心交互。
