...
Post-baseline source fixes now include the reproducible tokenizer build (PR #30), registration password-validation synchronization (PR #31), and targeted browser-runtime dependency updates (PR #32). Architecture issues #25 and #26 are closed. The architecture/product `CURRENT_STATUS` files still mention those two items as open, so those review repositories require a status-text refresh even though the executable source fixes are already merged.
