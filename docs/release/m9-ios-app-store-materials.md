...
- run `33425118337` verified the pinned Capacitor/Cordova `8.5.0` binary XCFramework artifacts from the Swift Package checksum path. The two official archives matched the package checksums, passed macOS `codesign --verify --deep --strict`, and reported the expected vendor signing identity (`Developer ID Application: Drifty Co. (9YN2HU59K8)`). Device framework privacy manifests were inspected as part of the same evidence. This establishes the current pinned vendor artifact signature, checksum and manifest evidence; it does not replace the publisher's final App Store submission checks or future Apple rule review;
