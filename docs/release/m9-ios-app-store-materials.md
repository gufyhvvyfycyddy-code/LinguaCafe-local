...
- run `33425118337` verified the binary-SDK signature/identity boundary for the exact Capacitor/Cordova `8.5.0` SwiftPM artifacts. The official release zips matched the checksums pinned by `capacitor-swift-pm`; macOS `codesign --verify --deep --strict` reported both XCFrameworks valid and satisfying their Designated Requirements; both are signed by `Developer ID Application: Drifty Co. (9YN2HU59K8)` with `TeamIdentifier=9YN2HU59K8` and secure timestamps. The iOS/device and iOS-Simulator variants contain structurally valid `PrivacyInfo.xcprivacy` files with the four expected root keys, empty accessed-API/collected-data/tracking-domain arrays and tracking disabled. Earlier archive run `33383576886` independently proved the Capacitor/Cordova framework privacy manifests are present in the actual Release archive;
...
These checks establish the repository-side candidate. Capacitor and Cordova are Apple-listed SDKs whose signatures are required when they are binary dependencies; the current pinned `8.5.0` artifacts have verified vendor identity/integrity plus the required manifest presence and archive structure. This evidence does not claim that GitHub CI substitutes for App Store Connect validation or that vendor manifest declarations can never change. Any future Capacitor/Cordova binary version change must re-run checksum, XCFramework signature/identity and SDK privacy-manifest verification before the release evidence is carried forward, and the deployment owner must re-check Apple's then-current third-party SDK rules at submission time.
...
- current chance-based-activity candidates are Gambling: `No`; Simulated Gambling: `None`; Contests: `None`; Loot Boxes: `No`. The mobile product has no wagering or simulated wagering, randomized paid-item boxes, competition event, leaderboard, prize or reward mechanism. Its ordinary personal study goal is not implemented as a contest or competitive event. Re-review these answers if a challenge, ranking, prize/reward, gambling or randomized-purchase feature is added;
