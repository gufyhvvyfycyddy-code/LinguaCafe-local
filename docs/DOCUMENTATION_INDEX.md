...
| `ai-study-card-v6-real-provider-implementation-plan.md` | Frozen plan; backend skeleton implemented disabled/fail-closed (2026-07-07) | V6-4/V6-5 real-provider implementation plan. Documents provider-preview route, backend adapter boundary, UI boundary, fail-closed behavior, and browser Network smoke requirements. Backend route skeleton exists but does not call a live provider. |
