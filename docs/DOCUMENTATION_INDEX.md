...
- H-10 iOS capability probe：历史 Windows-only 边界见 `docs/testing/h10-ios-capability-probe-2026-08-29.md`；2026-08-30 macOS continuation 见 `docs/testing/h10-macos-xcode-simulator-capability-continuation-2026-08-30.md`，已真实证明 Xcode 26.6/SwiftPM/Simulator build + iPhone 17 Pro boot/install/launch + rendered login-shell Accessibility/tap/input/local-HTTP warning，以及 same-runner testing backend 下的 rendered 登录、Keychain save/load、relaunch、server token/device owner、UI revoke/token clear、正式 Sense Review Good/Undo 与 exact FSRS restoration；run `33301226295` 又关闭真实 Reader 竖屏 token→canonical reading-origin Sense/source binding、existing-Sense `认识 / 记得`/`不认识`、横屏词组长按拖选和 MySQL/PAB cleanup 证据。剩余 Simulator `.txt` import、offline/reconnect sync 内容矩阵、physical device、signing/TestFlight/App Store 仍 DEFERRED
...
- H-GATE final audit：`docs/testing/h-gate-final-goal-completion-audit-2026-08-30.md`；Windows/Web/Android 已 stage-accepted，后续 cloud macOS continuation 又补齐 Xcode/SwiftPM/Simulator compile+launch、rendered login shell、authenticated Keychain/session lifecycle、正式 Sense Review Good/Undo 与 Reader touch/source-binding；E-08/H-10 的剩余 Simulator `.txt` import、offline/reconnect sync 内容矩阵及 physical device/signing/TestFlight/App Store 未清零，因此完整 A–H Goal 仍为 DEFERRED / Not Complete
