...
- H-10 iOS capability probe：历史 Windows-only 边界见 `docs/testing/h10-ios-capability-probe-2026-08-29.md`；2026-08-30/31 macOS continuation 见 `docs/testing/h10-macos-xcode-simulator-capability-continuation-2026-08-30.md`，已真实证明 Xcode 26.6/SwiftPM/Simulator build + iPhone 17 Pro boot/install/launch + rendered login-shell、authenticated Keychain lifecycle、正式 Sense Review Good/Undo 与 exact FSRS restoration、Reader token/Sense/source binding、真实 system Files `.txt` import；final full run `33350591521` 又关闭 offline article/review/audio cache、离线 Good 入队、relaunch persistence、reconnect exactly-once sync、stable empty queue、MySQL/PAB corroboration 与 strict cleanup。Simulator capability cluster 已 Stage Accepted；release-capability run `33355499203` 又验证 `arm64`、iPhone 17 Pro + `iPad Pro 13-inch (M5)` 串行 rendered shell，以及 `2064x2752` iPad screenshot；Stage Accepted H-10 净变化已由 squash integration commit `c2ef3da9` 正常进入当前 Goal 分支，临时 carrier 历史未进入 Goal；physical device、physical Keychain/haptics/notifications/audio/safe-area、Apple signing/archive、TestFlight/App Store/App Review、public Privacy/Support URL 仍 DEFERRED
...
- H-GATE final audit：`docs/testing/h-gate-final-goal-completion-audit-2026-08-30.md`；Windows/Web/Android 与 full required iOS Simulator cluster 均已 stage-accepted，后续 cloud macOS continuation 已补齐 Xcode/SwiftPM/Simulator compile+launch、rendered login/Keychain、正式 Sense Review Good/Undo、Reader touch/source-binding、真实 Files `.txt` import 和 final full run `33350591521` 的 offline/reconnect exactly-once sync。E-08/H-10 仅剩 physical device/physical Keychain/haptics/notifications/audio/safe-area、Apple signing/archive、TestFlight/App Store/App Review 未清零，因此完整 A–H Goal 仍为 DEFERRED / Not Complete
