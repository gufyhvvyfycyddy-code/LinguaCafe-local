...
- H-10 iOS capability probe：历史 Windows-only 边界见 `docs/testing/h10-ios-capability-probe-2026-08-29.md`；2026-08-30/31 macOS continuation 见 `docs/testing/h10-macos-xcode-simulator-capability-continuation-2026-08-30.md`。Xcode 26.6/SwiftPM/full Simulator、authenticated Keychain、Sense Review Good/Undo、Reader source binding、system Files `.txt` import 均已 Stage Accepted；final full run `33350591521` 关闭 offline/reconnect exactly-once sync；run `33355499203` 又验证 `arm64` 与串行 iPhone 17 Pro + 13-inch iPad rendered shell。随后 run `33359184066` 证明 compiled bundle 的 `ITSAppUsesNonExemptEncryption = NO`，privacy runs `33361617463` / `33362427450` 证明最终 App bundle 的六类 collected-data declaration、`UserDefaults / CA92.1`、non-tracking/empty tracking-domains，并再次通过 iPhone+iPad smoke/cleanup；privacy 净变化由 Goal commit `330caf56` 集成。后续 `33366809125` / `33383576886` 真实通过 unsigned Release `.xcarchive` 与 archive 内 App/Capacitor/Cordova privacy checks，`33386931423` 再通过 6.9-inch iPhone `1320x2868` + 13-inch iPad `2064x2752` JPEG/no-alpha 截图技术门；本轮状态收口前已验证 Goal baseline=`8e882b75`；实时 tip 仍以 fresh Git preflight 为准。signed physical device、physical Keychain/haptics/notifications/audio/safe-area、Apple-team signing + signed archive / Organizer `Validate App`、TestFlight/App Store processing/App Review、final screenshot selection/upload、public Privacy/Support URL 与 publisher-owned final store answers 仍 DEFERRED
...
- H-GATE final audit：`docs/testing/h-gate-final-goal-completion-audit-2026-08-30.md`；Windows/Web/Android 与 full required iOS Simulator cluster 均已 stage-accepted，repository-side `arm64`、export compliance、final Privacy Manifest、unsigned Release archive、SDK privacy manifests 与 App Store screenshot technical classes 也已由 Xcode 26.6 真实通过。E-08/H-10 仅剩 signed physical device / physical Keychain-haptics-notifications-audio-safe-area、Apple-team signing + signed archive / Organizer `Validate App`、TestFlight/App Store processing、final marketing screenshot selection/upload、public Privacy/Support URLs、publisher-owned final store answers 与 App Review 未清零，因此完整 A–H Goal 仍为 DEFERRED / Not Complete
