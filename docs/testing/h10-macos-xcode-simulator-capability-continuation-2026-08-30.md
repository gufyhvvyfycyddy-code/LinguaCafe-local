...
## Capacitor/Cordova required binary-SDK signature evidence — 2026-09-01

Apple lists Capacitor and Cordova among the third-party SDKs that require privacy manifests for applicable submissions and also require SDK signatures when used as binary dependencies. LinguaCafe's pinned `capacitor-swift-pm` `8.5.0` package uses binary XCFramework targets, so this boundary applies independently of the app's later Apple-team signing.

Isolated macOS validation run `33425118337` downloaded the exact official Capacitor `8.5.0` binary assets. Their SHA-256 values matched the vendor SwiftPM checksums (`357220fe...512130` for Capacitor and `a3dc72b5...83a10` for Cordova). `codesign --verify --deep --strict` reported both XCFramework bundles valid on disk and satisfying their Designated Requirements. Both report `Developer ID Application: Drifty Co. (9YN2HU59K8)`, `TeamIdentifier=9YN2HU59K8`, and a secure timestamp of 2026-07-31. The run also verified structurally valid `PrivacyInfo.xcprivacy` files in the iOS device and iOS Simulator framework variants for both SDKs; their four expected root keys are present, accessed-API/collected-data/tracking-domain arrays are empty and tracking is disabled. Earlier Release archive run `33383576886` independently proved the Capacitor/Cordova framework manifests are included in the archived app.

This closes the current pinned `8.5.0` repository-side binary-SDK signature/identity and manifest-presence/structure check. It does not claim that GitHub CI substitutes for App Store Connect validation or that the vendor's privacy declarations can never change. Any future Capacitor/Cordova binary version change must repeat the official-asset checksum, XCFramework signature/identity and privacy-manifest verification, and the deployment owner must re-check Apple's then-current third-party SDK rules at submission time. This evidence must not be inherited across versions. It does not provide LinguaCafe app signing, provisioning, signed Release archive, TestFlight, App Store Connect or App Review evidence.
