...
No endpoint, payload, access rule, database schema, ReviewLog behavior, lifecycle/FSRS behavior or write semantics changed. Automated guards, grouped related Feature tests, the Unit suite and the frontend build passed.

Authenticated Chrome acceptance completed on 2026-07-16 at 1920×1080 and 900×900. Opening Card Info issued exactly one canonical `GET /review-cards/manage/{id}/detail`; overview/history/diagnosis tab changes issued no additional request. No legacy single-card `/logs`, `/lifecycle-events` or `/leech` request appeared. The page-level `/review-cards/manage/leech-summary` request remains an independent list summary. Slow 3G close/reopen and card 156 → 157 switching stabilized on card 157 without stale overwrite. Deep-link source hint, report return, user-triggered source context, clean Console and no horizontal overflow passed. Read-only use did not write ReviewLog or change FSRS data.

Phase 3A is **Accepted / Production Closed**. Phase 3B-1 is only **Authorized Next / Not Started**; this acceptance does not implement or automatically enter it.
