...
assert.match(readerUiTest, /XCUIApplication\(bundleIdentifier: "com\.apple\.DocumentsApp"\)/);
assert.match(readerUiTest, /LC_IMPORT_FILE_NAME/);
assert.match(readerUiTest, /app\.buttons\["导入到服务器"\]/);
...
assert.doesNotMatch(readerUiTest, /evaluateJavaScript|evaluateScript|__lc_test_import/);
...
assert.match(readerAcceptanceWorkflow, /TEST_RUNNER_LC_IMPORT_FILE_NAME=/);
assert.match(readerAcceptanceWorkflow, /TEST_RUNNER_LC_IMPORT_BOOK_NAME=/);
assert.match(readerAcceptanceWorkflow, /com\.apple\.DocumentsApp/);
...
assert.match(readerAcceptanceWorkflow, /run_ui_test testTextImportThroughSystemFilesPicker 300/);
...
assert.match(readerAcceptanceWorkflow, /grep -q 'api\/v1\/mobile\/imports\/text'/);
