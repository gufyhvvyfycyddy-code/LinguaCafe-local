...
const mobilePrivacy = read('mobile', 'src', 'privacy.ts');
...
const versionCandidate = listingValue('Version candidate');
const buildCandidate = listingValue('Build candidate');
const keywords = listingValue('Keywords');
const promotionalText = storeMaterials.match(/^- Promotional text: `([\s\S]*?)`/m)?.[1].replace(/\s+/g, ' ').trim() ?? '';
const description = storeMaterials.match(/^Description:\s*\r?\n\r?\n((?:>.*(?:\r?\n|$))+)/m)?.[1].replace(/^>\s?/gm, '').replace(/\s+/g, ' ').trim() ?? '';
const reviewNotes = storeMaterials.match(/^## Review notes draft\s*\r?\n\r?\n([\s\S]*?)\r?\n## /m)?.[1].trim() ?? '';
assert.ok(appName.length >= 2 && appName.length <= 30, `App Store app name must be 2-30 characters, got ${appName.length}`);
assert.ok(subtitle.length > 0 && subtitle.length <= 30, `App Store subtitle must be 1-30 characters, got ${subtitle.length}`);
assert.equal(versionCandidate, '1.0.0');
assert.equal(buildCandidate, '1');
const marketingVersions = new Set([...project.matchAll(/MARKETING_VERSION = ([^;]+);/g)].map(match => match[1]));
const buildVersions = new Set([...project.matchAll(/CURRENT_PROJECT_VERSION = ([^;]+);/g)].map(match => match[1]));
assert.deepEqual(marketingVersions, new Set(['1.0.0']));
assert.deepEqual(buildVersions, new Set(['1']));
...
assert.match(promotionalText, /iPhone and iPad/);
...
assert.ok((ui.match(/mobilePrivacyPolicyHtml\(\)/g) ?? []).length >= 2, 'iOS privacy policy copy must remain reachable from login and settings');
...
assert.match(xcodeCapabilityWorkflow, /Print :CFBundleShortVersionString[^\n]*= \"1\.0\.0\"/);
assert.match(xcodeCapabilityWorkflow, /Print :CFBundleVersion[^\n]*= \"1\"/);
...
assert.match(unsignedArchiveBlock, /Print :CFBundleDisplayName/);
assert.match(unsignedArchiveBlock, /Print :CFBundleShortVersionString[^\n]*= \"1\.0\.0\"/);
assert.match(unsignedArchiveBlock, /Print :CFBundleVersion[^\n]*= \"1\"/);
assert.match(unsignedArchiveBlock, /Archived PrivacyInfo\.xcprivacy differs semantically from source/);
assert.match(unsignedArchiveBlock, /Frameworks\/Capacitor\.framework\/PrivacyInfo\.xcprivacy/);
assert.match(unsignedArchiveBlock, /Frameworks\/Cordova\.framework\/PrivacyInfo\.xcprivacy/);
assert.match(unsignedArchiveBlock, /SDK privacy manifest missing required keys/);
assert.match(unsignedArchiveBlock, /SDK privacy manifest must keep tracking disabled/);
assert.match(unsignedArchiveBlock, /SDK privacy manifest key must be an array/);
...
assert.match(xcodeCapabilityWorkflow, /d\.get\("name"\) == "iPhone 17 Pro Max"/);
...
assert.match(xcodeCapabilityWorkflow, /iphone-69-login-shell\.jpg/);
assert.match(xcodeCapabilityWorkflow, /2064x2752\|2048x2732/);
assert.match(xcodeCapabilityWorkflow, /ipad-13-login-shell\.jpg/);
assert.equal((xcodeCapabilityWorkflow.match(/screenshot --type=jpeg/g) ?? []).length, 2);
assert.equal((xcodeCapabilityWorkflow.match(/sips -g hasAlpha/g) ?? []).length, 2);
assert.equal((xcodeCapabilityWorkflow.match(/test "\$HAS_ALPHA" = "no"/g) ?? []).length, 2);
...
assert.match(xcodeCapabilityWorkflow, /13-inch iPad screenshot:/);
...
assert.match(readerAcceptanceWorkflow, /run_ui_test testLoginPersistsAuthenticatedSession 360/);
assert.ok((readerAcceptanceWorkflow.match(/default: full/g) ?? []).length >= 2);
const loginRunIndex = readerAcceptanceWorkflow.indexOf('run_ui_test testLoginPersistsAuthenticatedSession 360');
...
const corroborationBlock = readerAcceptanceWorkflow.match(/- name: Corroborate canonical Reader source binding in MySQL and PAB[\s\S]*?- name: Cleanup PAB, lease, database service, and simulator/)?.[0] ?? '';
