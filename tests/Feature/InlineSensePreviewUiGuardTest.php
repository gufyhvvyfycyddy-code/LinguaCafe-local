...
/**
 * GLM-ReadingInlinePreview-First-1 (sub-stage 5, +100%)
 *
 ...
 *  2. safety 文案 exists ("不会写入复习记录" / "不会改变 FSRS");
 *  3. no "立即评分" copy;
 *  4. no "写入复习记录" copy;
 *  5. no "AI 已判断" copy;
 *  6. legacy entry copy not re-introduced;
 *  7. component does not trigger a rating route (no /reviews/senses/.../rate
 *     or /reviews/rate POST in InlineSensePreviewPanel.vue).
 */
class InlineSensePreviewUiGuardTest extends TestCase
{
    ...
    {
        $contents = file_get_contents($this->panelPath);
        $blocked = ['AI 已判断', 'AI 判断', 'AI已判断'];
        foreach ($blocked as $copy) {
            $this->assertStringNotContainsString($copy, $contents, 'panel must not show AI-judgment copy.');
        }
    }

    public function test_panel_does_not_trigger_rating_route(): void
    {
        $contents = file_get_contents($this->panelPath);
        $blockedRoutes = [
            '/reviews/senses/',
        ...
        foreach ($blockedRoutes as $route) {
            $this->assertStringNotContainsString($route, $contents, 'panel must not trigger rating route or service.');
        }
    }

    public function test_panel_choice_buttons_only_set_local_state(): void
    {
        $contents = file_get_contents($this->panelPath);
        // The choice buttons must call setChoice (local), not axios.post / axios.put.
        $this->assertStringContainsString('setChoice', $contents, 'panel must have setChoice method.');
        $this->assertStringNotContainsString('axios.post', $contents, 'panel must not POST any choice to backend.');
        $this->assertStringNotContainsString('axios.put', $contents, 'panel must not PUT any choice to backend.');
    }
