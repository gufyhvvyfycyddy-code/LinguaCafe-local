 ...
 * workload. This slice owns the manifest contract; it does not claim that
 * these records have been inserted into a database.
 *
 * @return list<array{email:string,password:string,chapter_id:int,lemma:string,language:string,review_card_id:int}>
 */
function h02PrepareFixtureRows(int $vus): array
{
    ...
    for ($index = 1; $index <= $vus; $index++) {
        ...
        $rows[] = [
            ...
            'chapter_id' => 200000 + $index,
            'lemma' => "h02-vu-{$suffix}",
            'language' => 'en',
            'review_card_id' => 300000 + $index,
        ];
    }
